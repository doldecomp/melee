                                           ...
                                           void* func)
{
    ...
    handle =
        (void*) OSAllocFromHeap(__OSCurrHeap, sizeof(struct STRUCT_DEMOWIN));
    ...
    handle->x_cal =
        (((handle->pixel_width - (handle->char_width * 8)) + 1) / 2);
    handle->y_cal =
        (((handle->pixel_height - 7) - (handle->char_height * 8)) / 2);
    ...
    handle->flags = 0;
    handle->priority = 0;
    handle->buffer = (void*) OSAllocFromHeap(
        __OSCurrHeap, handle->total_lines * handle->char_width);
    ASSERTMSGLINE(0xEE, handle->buffer,
                  "DEMOWinCreateWindow(): Unable to allocation buffer!\n");
    memset(handle->buffer, ' ',
           handle->total_lines *
               handle->char_width); // set to all empty spaces
...
void DEMOWinRefresh()
{
    ...
    ASSERTMSGLINE(0x30D, __first_node,
                  "DEMOWinRefresh(): Windowlist is empty!\n");
    for (ptr = __first_node; ptr; ptr = ptr->next) {
        if (ptr->flags & DEMOWIN_FLAGS_OPENED) {
            ...
            // macro?
            do {
                // would initialize on init, but DWARF order for 2nd macro
                // suggests they didnt init on same declare.
                unsigned char r1;
                unsigned char g1;
                unsigned char b1;
                unsigned char r2;
            ...
            DEMOPrintf(ptr->x1 + 2, ptr->y1, ptr->priority, "%s",
                       ptr->caption);
            if (ptr->cursor_line >= 0) {
                ...
                do {
                    ...
                    curr_y -=
                        ((ptr->char_height - 1) * 8) - ptr->cursor_line * 8;
...
static void __DEMOWin_delete_node(struct STRUCT_DEMOWIN* handle)
{
    ...
    ASSERTMSGLINE(0x424, handle,
                  "__delete_node(): you're deleting a NULL node!\n");
...
static void __DEMOWin_puts_n(signed short x, signed short y, signed short z,
                             unsigned short n, char* string)
{
    long s;
    long t;
    long w;
    ...
    while (!exit_flag) {
        ...
        case 1:
            ...
            while (menu->items[menu->curr_pos].flags & 9) {
                menu->curr_pos = ((menu->curr_pos - 1 + menu->num_items) %
                                  menu->num_items) &
                                 0xFFFF;
            }
            if (menu->cb_move) {
...
long DEMOWinListScrollList(struct STRUCT_LISTBOX* list, unsigned long dir)
{
    ...
    case 1:
        if (list->display_pos) {
            list->display_pos =
                (u16) ((list->display_pos - 1 + list->num_items) %
                       list->num_items);
        }
        break;
    case 2:
