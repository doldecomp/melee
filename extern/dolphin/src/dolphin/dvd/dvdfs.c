...
OSThreadQueue __DVDThreadQueue;        // size: 0x8, address: 0x14
unsigned long __DVDLongFileNameFlag;   // size: 0x4, address: 0x18
...
static BOOL DVDConvertEntrynumToPath(s32 entrynum, char* path, u32 maxlen)
{
    ...
    ASSERTMSG1LINE(
        ...
        entrynum);
    ASSERTMSG1LINE(0x257, maxlen > 1,
                   "DVDConvertEntrynumToPath: maxlen should be more than 1 "
                   "(%d is specified)",
                   maxlen);
    ASSERTMSGLINE(0x25C, entryIsDir(entrynum),
                  "DVDConvertEntrynumToPath: cannot convert an entry num for "
                  "a file to path  ");
                 ...
                 long offset, long prio)
{
    ...
    block = &fileInfo->cb;
    result =
        DVDReadAbsAsyncPrio(block, addr, length, fileInfo->startAddr + offset,
                            cbForReadSync, prio);
    if (result == 0) {
        return -1;
    }
                     ...
                     void (*callback)(long, struct DVDFileInfo*), long prio)
{
    ...
    DVDSeekAbsAsyncPrio(&fileInfo->cb,
                        (u32) (char*) fileInfo->startAddr + offset,
                        cbForSeekAsync, prio);
    return 1;
}
...
long DVDSeekPrio(struct DVDFileInfo* fileInfo, long offset, long prio)
{
    ...
    block = &fileInfo->cb;
    result =
        DVDSeekAbsAsyncPrio(block, (u32) (char*) fileInfo->startAddr + offset,
                            cbForSeekSync, prio);
    if (!result) {
        return -1;
    }
...
int DVDOpenDir(char* dirName, DVDDir* dir)
{
    ...
    ASSERTMSG2LINE(0x409, entry >= 0,
                   ...
                   dirName, (DVDGetCurrentDir(currentDir, 128), currentDir));
    ASSERTMSG1LINE(
        0x40E, entryIsDir(entry),
        "DVDOpendir(): file '%s' is specified as a directory name  ", dirName);
...
s32 DVDPrepareStream(DVDFileInfo* fileInfo, u32 length, u32 offset)
{
    ...
    if (!Is32KBAligned(length)) {
        OSPanic(__FILE__, 0x4C9,
                "DVDPrepareStream(): Specified length (0x%x) is not a "
                "multiple of 32768(32*1024)",
                length);
    }
