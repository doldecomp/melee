...
#define CHECK_LISTPTR(line, list)                                             \
    ASSERTMSGLINE(line, (list) != NULL,                                       \
                  "GXSetVtxAttrFmt: list pointer is NULL")
...
static inline void SETVAT(u32* va, u32* vb, u32* vc, GXAttr attr,
                          GXCompCnt cnt, GXCompType type, u8 shft)
{
    switch (attr) {
    case GX_VA_POS:
