...
static char str_reg_field_out_of_range[] =
...
void GXEnableBreakPt(void* break_pt)
{
    ...
    __GXFifoReadDisable();
    ASSERTMSGLINE(
        0x44A, (u8*) break_pt >= GPFifo->base && (u8*) break_pt <= GPFifo->top,
...
OSThread* GXSetCurrentGXThread(void)
{
    ...
    ASSERTMSGLINE(0x532, !GXOverflowSuspendInProgress,
                  "GXSetCurrentGXThread: Two threads cannot generate GX "
...
static char str_failed_assertion_offset[] =
    "Failed assertion OFFSET(ptr, 32) == 0";
static char str_failed_assertion_pipenotredirected[] =
    "Failed assertion !IsWGPipeRedirected";
static char str_failed_assertion_piperedirected[] =
    "Failed assertion IsWGPipeRedirected";
...
void GXRestoreWriteGatherPipe(void)
{
    ...
    // SET_REG_FIELD(0x5FB, reg, 25, 5, ((u32)CPUFifo->wrPtr & 0x3FFFFFFF) >>
    // 5);
    reg &= 0xFBFFFFFF;
    __piReg[5] = reg;
    if (CPGPLinked) {
