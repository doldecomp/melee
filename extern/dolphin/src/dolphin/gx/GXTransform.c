...
static asm void WriteMTXPS3x3(register f32 mtx[3][3],
                              register volatile f32* dest)
{
    // clang-format off
    psq_l f0, 0x00(mtx), 0, qr0
...
#define SET_REG_FIELD_(line, reg, size, shift, val)                           \
    do {                                                                      \
        ASSERTMSGLINE(line, ((s32) (val) & ~((1 << (size)) - 1)) == 0,        \
...
void GXLoadTexMtxImm(f32 mtx[][4], u32 id, GXTexMtxType type)
{
    ...
    if (id >= GX_PTTEXMTX0) {
        ...
        ASSERTMSGLINE(0x2CC, type == GX_MTX3x4,
                      "GXLoadTexMtx: Invalid matrix type");
    } else {
        addr = id * 4;
    }
...
void GXLoadTexMtxIndx(u16 mtx_indx, u32 id, GXTexMtxType type)
{
    ...
    if (id >= GX_PTTEXMTX0) {
        ...
        ASSERTMSGLINE(0x314, type == GX_MTX3x4,
                      "GXLoadTexMtx: Invalid matrix type");
    } else {
        offset = id * 4;
    }
...
void GXGetScissor(u32* left, u32* top, u32* wd, u32* ht)
{
    ...
    ASSERTMSGLINE(0x3DD, left && top && wd && ht,
                  "GXGet*: invalid null pointer");
...
void GXSetScissorBoxOffset(s32 x_off, s32 y_off)
{
    ...
    ASSERTMSGLINE(0x3FE, (u32) (x_off + 340) < 2048,
                  "GXSetScissorBoxOffset: x offset > 2048");
