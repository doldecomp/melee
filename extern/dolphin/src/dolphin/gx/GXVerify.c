...
char* __gxvWarnings[113] = {
    ...
    "Minification filter for texture %ld is not compatible with color index "
    "texture format.",
    ...
    "Color arg %s in TEV stage %ld accesses %s register %ld, which may be "
    "dirty.",
    "Alpha arg %s in TEV stage %ld accesses alpha register %ld, which may be "
    "dirty.",
    "Color arg %s in TEV stage %ld accesses %s register %ld, which was last "
    "clamped linear high. Possible wrap-around effect.",
    ...
    "Final color stage has linear high clamping possible color wrap-around "
    "effect.",
    "Final alpha stage has linear high clamping possible alpha wrap-around "
    "effect.",
    ...
    "Multisample locations for pixel %ld are not ordered correctly for "
    "antialias filter.",
    ...
    "XF is not expecting host normals, binormals and tangents but cp is "
    "sending them",
    ...
    "tangents",
    "XF is expecting host normals, binormals and tangents but cp is only "
    "sending normals",
    "XF is expecting host normals, binormals and tangents but cp is not "
    "sending them",
    "Invalid value (%d) for INVERTEXSPEC_REG.host_normal",
    "XF expecting %d host textures but CP is only sending %d",
    "Regular texture %d specifying source row of geometry, but this is not "
    "getting sent by the CP",
    "Regular texture %d specifying source row of normals, but this is not "
    "getting sent by the CP",
    "Regular texture %d specifying source row of colors, but color0 is not "
    "getting sent by the CP",
    "Regular texture %d specifying source row of colors, but color1 is not "
    "getting sent by the CP",
    "Regular texture %d specifying source row of binormal or tangent, but "
    "these are not getting sent by the CP",
    ...
    "Texture texgen types are out of order. Must be defined as regular if "
    "any, then bumpmap if any, then color if any",
    "Bumpmap textures are defined, which requires the binormals and tangents "
    "to be transformed by a normal matrix, but MATRIXINDEX0_REG.geom is set "
    "to an invalid value (%d)",
    "Bumpmap %d (texture %d) is referencing texture %d as a source texture, "
    "which is not of texgen type regular",
    "Bumpmap %d (texture %d) using light source %d, but light's %c position "
    "is not defined",
    "Texture %d is defined as texgen type Bumpmap, but cp is not sending "
    "binormals and tangents",
    "Invalid regular texture number (%d)",
    "Regular texture %d specifying a source row of %d which only has 2 "
    "elements, but an input form of ABC1",
    "Output XF colors or color textures enabled, but register address 0x%04x "
    "uninitialized (%s)",
    "Output XF colors or color textures enabled, "
    "COLOR%dCNTRL_REG.material_src == REGISTER, but Material %d register "
    "(0x%04x) is not initialized",
    "Output XF colors or color textures enabled, COLOR%dCNTRL_REG.ambient_src "
    "== REGISTER, but Ambient %d register (0x%04x) is not initialized",
    "%sCNTRL_REG.attenselect == SPECULAR but %sCNTRL_REG.diffuseatten != "
    "NL_ONE",
    "Color %d lighting requires a normal, but cp is not sending it",
    "Color %d lighting requires the normal to be transformed by a normal "
    "matrix, but MATRIXINDEX0_REG.geom is set to an invalid value (%d)",
    ...
    "%d regular textures active, but MatrixIndex1 register (0x%04x) "
    "uninitialized",
    "gen_mode register not initialized",
    "Number of XF output textures does not match what downstream units are "
    "expecting",
    "Number of XF output colors does not match what downstream units are "
    "expecting",
    ...
    "Bumpmap textures are defined, which requires the normal matrix values "
    "pointed to by MATRIXINDEX0_REG.geom (%d) to be loaded, however...",
    "Texture %d is a regular texture, which requires that the matrix values "
    "pointed to by MATRIXINDEX0_REG.tex%d (%d) must be loaded, however...",
    "Light %d is being referenced, however...",
    "Color %d lighting requires the normal matrix values pointed to by "
    "MATRIXINDEX0_REG.geom (%d) to be loaded, however...",
