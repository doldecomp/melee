...
static struct OSThread* SetEffectivePriority(struct OSThread* thread,
                                             long priority);
                                   ...
                                   void* stack, unsigned long stackSize)
{
    if (idleFunction) {
        if (IdleThread.state == 0) {
            OSCreateThread(&IdleThread, (void*) idleFunction, param, stack,
                           stackSize, 0x1F, 1);
...
long OSCheckActiveThreads()
{
    ...
    for (prio = 0; prio <= 0x1F; prio++) {
        if (RunQueueBits & (1 << (0x1F - prio))) {
            ASSERTREPORT(0x566, RunQueue[prio].head != NULL &&
                                    RunQueue[prio].tail != NULL);
        } else {
            ASSERTREPORT(0x56B, RunQueue[prio].head == NULL &&
                                    RunQueue[prio].tail == NULL);
        }
    ...
    while (thread) {
        ...
        case 8:
            ...
            break;
        default:
            OSReport("OSCheckActiveThreads: Failed. unkown thread state (%d) "
                     "of thread %p\n",
                     thread->state, thread);
