...
static volatile unsigned long CurrToken =
    0x0000FFFF; // size: 0x4, address: 0x4
...
unsigned long PERFInit(unsigned long numSamples,
                       unsigned long numFramesHistory, unsigned long numTypes,
...
static void PERFTokenCallback(unsigned short token)
{
        ...
        (PERFCurrSample == 0))
    {
        ...
        if (token >= 0xF000) {
            ...
            PERFFrames[PERFCurrFrame].samples[sample].gpTimeStampEnd =
                PPCMfpmc4();
            PERFFrames[PERFCurrFrame].samples[sample].cacheMisses[1] =
                PPCMfpmc3();
            PERFFrames[PERFCurrFrame].samples[sample].instructions[1] =
                PPCMfpmc1();
            PERFGetAllMemMetrics(&PERFFrames[PERFCurrFrame].samples[sample],
                                 1);
            if (CurrAutoSample >= 0) {
        ...
        PERFFrames[PERFCurrFrame].samples[sample].gpTimeStampStart =
            PERFFrames[PERFCurrFrame].samples[sample].origgpStart =
                PPCMfpmc4();
...
void PERFEventStart(unsigned char id)
{
    ...
    if (sample < 0) {
        ...
        case PERF_CPU_EVENT:
            ...
            break;
        default:
            OSReport("PERF : Unknown event type for ID %d - possibly out of "
                     "memory\n",
                     id);
            break;
        }
...
void PERFEventEnd(unsigned char id)
{
    ...
    case PERF_GP_EVENT:
        GXSetDrawSync(((u16) sample + 0x10000) +
                      (((u16) magic << 8) & 0xFF00) - 0x1000);
        break;
    case PERF_CPU_GP_EVENT:
        GXSetDrawSync(((u16) sample + 0x10000) +
                      (((u16) magic << 8) & 0xFF00) - 0x1000);
    case PERF_CPU_EVENT:
        PERFFrames[PERFCurrFrame].samples[sample].cpuTimeStampEnd =
            PPCMfpmc4();
        PERFFrames[PERFCurrFrame].samples[sample].cacheMisses[3] = PPCMfpmc3();
        PERFFrames[PERFCurrFrame].samples[sample].instructions[3] =
            PPCMfpmc1();
        break;
    }
    PERFEvents[id].currSample = -1;
...
static void PERFStartAutoSample()
{
    ...
    PERFGetAllMemMetrics(&PERFFrames[PERFCurrFrame].samples[CurrAutoSample],
                         0);
    PERFFrames[PERFCurrFrame].samples[CurrAutoSample].gpTimeStampStart =
        PPCMfpmc4();
    PERFFrames[PERFCurrFrame].samples[CurrAutoSample].gpTimeStampEnd = 0;
    PERFFrames[PERFCurrFrame].samples[CurrAutoSample].cacheMisses[0] =
        PPCMfpmc3();
    PERFFrames[PERFCurrFrame].samples[CurrAutoSample].instructions[0] =
        PPCMfpmc1();
}
...
static void PERFEndAutoSample()
{
    if (CurrAutoSample >= 0) {
        ...
        PERFGetAllMemMetrics(
            &PERFFrames[PERFCurrFrame].samples[CurrAutoSample], 1);
    ...
    if (PERFCurrSample != 0) {
        if (CurrToken < 0xFFFF) {
            ...
            PERFFrames[PERFCurrFrame].samples[newsample].instructions[1] =
                PPCMfpmc1();
            PERFGetAllMemMetrics(&PERFFrames[PERFCurrFrame].samples[newsample],
                                 1);
