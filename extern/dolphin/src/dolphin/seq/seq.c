...
static u8 __SEQMidiEventLength[128] = {
    ...
    0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
    0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02,
    0x02, 0x02, 0x02, 0x02, 0x00, 0x00, 0x02, 0x01, 0x00, 0x00, 0x00, 0x00,
...
static void __SEQReadHeader(SEQSEQUENCE* sequence, u8* midiStream)
{
    ...
    read = midiStream;
    ASSERTMSGLINE(0x188, *(u32*) read == 'MThd',
    ...
    read += 2;
    ASSERTMSGLINE(0x197, sequence->timeFormat >= 0,
                  "!!!SEQ does not support SMPTE time!!!\n");
    bytes -= 6;
    read += bytes;
    switch (fileType) {
    case 0:
    ...
    case 1:
        ASSERTMSGLINE(
            0x1AD, sequence->nTracks < 0x40,
            "exceeded SEQ_MAX_TRACKS, please increase SEQ_MAX_TRACKS\n");
...
void SEQAddSequence(SEQSEQUENCE* sequence, u32* midiStream, void* wt,
                    u32 aramBase, u32 priorityVoiceAlloc, u32 priorityNoteOn,
                    u32 priorityNoteRelease)
{
    ...
    SYNInitSynth(&sequence->synth, wt, aramBase, priorityVoiceAlloc,
                 priorityNoteOn, priorityNoteRelease);
    sequence->state = 0;
    for (i = 0; i < 0x80; i++) {
...
void SEQSetTempo(SEQSEQUENCE* sequence, u32 trackIndex, float bpm)
{
    ...
    ASSERTLINE(0x2BD, (trackIndex < sequence->nTracks) ||
                          (trackIndex == SEQ_ALL_TRACKS));
    if (trackIndex == -1) {
        for (i = 0; i < sequence->nTracks; i++) {
