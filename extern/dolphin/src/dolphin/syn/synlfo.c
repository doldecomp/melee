...
void __SYNRunLfo(struct SYNVOICE* voice)
{
    ...
    if (voice->lfoDelay != 0) {
        ...
        lfoModWheel =
            __SYNn128[voice->synth->controller[voice->midiChannel][1]];
        voice->lfoAttn = (lfoAmplitude * (voice->lfoAttn_ +
                                          (voice->lfoModAttn * lfoModWheel)));
        voice->lfoCents =
            (lfoAmplitude *
             (voice->lfoCents_ + (voice->lfoModCents * lfoModWheel)));
    }
}
