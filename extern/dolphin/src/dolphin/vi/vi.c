...
static void __VIRetraceHandler(__OSInterrupt unused, OSContext* context)
{
    ...
    else if (changed != 0)
    {
        dbgCount++;
        if (dbgCount > 60) {
            OSReport("Warning: VIFlush() was not called for 60 frames "
                     "although VI settings were changed\n");
            dbgCount = 0;
        }
    }
#endif
...
static void AdjustPosition(u16 acv)
{
    ...
    HorVer.AdjustedDispPosY =
        MAX((s16) HorVer.DispPosY + displayOffsetV, frac);
...
void VIConfigurePan(u16 xOrg, u16 yOrg, u16 width, u16 height)
{
    ...
    if (HorVer.FBMode == VI_XFBMODE_DF) {
        ASSERTMSGLINEV(0x6A1, (height & 1) == 0,
                       "VIConfigurePan(): Odd number(%d) is specified to "
                       "height when DF XFB mode\n",
                       height);
    }
#endif
    enabled = OSDisableInterrupts();
...
u32 VIGetTvFormat(void)
{
    ...
    ASSERTMSGLINE(0x80D, format == 0 || format == 1 || format == 2,
                  "VIGetTvFormat(): Wrong format is stored in lo mem. Maybe "
                  "lo mem is trashed");
