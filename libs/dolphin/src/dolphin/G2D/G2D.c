...
void G2DInitSprite(G2DSprite* sprite)
{
    ...
    sprite->rS1 =
        rInvWidth * (((f32) sprite->nTlcS + (f32) sprite->nWidth) - 0.5f);
    sprite->rT0 = (0.5f + (f32) sprite->nTlcT) * rInvHeight;
    sprite->rT1 =
        rInvHeight * (((f32) sprite->nTlcT + (f32) sprite->nHeight) - 0.5f);
}
...
static inline void FillSection(G2DLayer* layer, s8* aSortBuffer,
                               s32* nScanLine, s32 nEvent, s16* nIdx, s32* nL,
                               s32* nR, f32* rLeft, f32* rRight, f32 rStep0,
                               f32 rStep1, s32 nMapX, s32 nMapY)
{
    long nHMask;
    long nVMask;
    long nI;
...
void G2DDrawLayer(G2DLayer* layer, s8* aSortBuffer)
{
    ...
    if (!layer->bWrap) {
        ...
        f32 rSplitX = 0.5f * (glob.rWorldX +
                              (f32) (layer->nTileWidth * (1 << layer->nHS)));
        f32 rSplitY = 0.5f * (glob.rWorldY +
                              (f32) (layer->nTileHeight * (1 << layer->nVS)));
    ...
    FillSection(layer, aSortBuffer, &nScanLine, nEvent0, &nIdx, &nL, &nR,
                &rLeft, &rRight, rStep0, rStep1, nLocalMapX, nLocalMapY);
    ...
    FillSection(layer, aSortBuffer, &nScanLine, nEvent1, &nIdx, &nL, &nR,
                &rLeft, &rRight, rMid, rMid, nLocalMapX, nLocalMapY);
    ...
    FillSection(layer, aSortBuffer, &nScanLine, nEvent2 + 1, &nIdx, &nL, &nR,
                &rLeft, &rRight, rStep1, rStep0, nLocalMapX, nLocalMapY);
