...
void GXLoadTexObjPreLoaded(GXTexObj* obj, GXTexRegion* region, GXTexMapID id)
{
    ...
    ASSERTMSGLINEV(0x401, id < 8, "%s: invalid texture map ID",
                   "GXLoadTexObj");
...
void GXLoadTexObj(GXTexObj* obj, GXTexMapID id)
{
    ...
    ASSERTMSGLINEV(0x433, id < 8, "%s: invalid texture map ID",
                   "GXLoadTexObj");
    ASSERTMSGLINEV(0x438, gx->texRegionCallback,
                   "%s: Tex/Tlut Region Callback not set", "GXLoadTexObj");
    r = gx->texRegionCallback(obj, id);
    ASSERTMSGLINEV(0x43A, r, "%s: Tex/Tlut Region Callback returns NULL",
                   "GXLoadTexObj");
...
void GXLoadTlut(GXTlutObj* tlut_obj, u32 tlut_name)
{
    ...
    ASSERTMSGLINEV(0x4A7, gx->tlutRegionCallback,
                   "%s: Tex/Tlut Region Callback not set", "GXLoadTlut");
    r = (__GXTlutRegionInt*) gx->tlutRegionCallback(tlut_name);
    ASSERTMSGLINEV(0x4A9, r, "%s: Tex/Tlut Region Callback returns NULL",
                   "GXLoadTlut");
                          ...
                          GXTexCacheSize size_odd)
{
    ...
    ASSERTMSGLINEV(0x4DC, (tmem_even & 0x1F) == 0,
                   "%s: %s pointer not aligned to 32B", "GXInitTexCacheRegion",
                   "tmem even");
    ASSERTMSGLINEV(0x4DE, (tmem_odd & 0x1F) == 0,
                   "%s: %s pointer not aligned to 32B", "GXInitTexCacheRegion",
                   "tmem odd");
    switch (size_even) {
    ...
    case GX_TEXCACHE_512K:
        ...
        break;
    default:
        ASSERTMSGLINEV(0x4E6, 0, "%s: Invalid %s size", "GXInitTexCacheRegion",
                       "tmem even");
        break;
    }
    t->image1 = 0;
    ...
    case GX_TEXCACHE_NONE:
        ...
        break;
    default:
        ASSERTMSGLINEV(0x4F6, 0, "%s: Invalid %s size", "GXInitTexCacheRegion",
                       "tmem odd");
        break;
    }
    t->image2 = 0;
                            ...
                            u32 tmem_odd, u32 size_odd)
{
    ...
    ASSERTMSGLINEV(0x51E, (tmem_even & 0x1F) == 0,
                   "%s: %s pointer not aligned to 32B",
                   "GXInitTexPreLoadRegion", "tmem even");
    ASSERTMSGLINEV(0x520, (tmem_odd & 0x1F) == 0,
                   "%s: %s pointer not aligned to 32B",
                   "GXInitTexPreLoadRegion", "tmem odd");
    ASSERTMSGLINEV(0x522, (size_even & 0x1F) == 0,
                   "%s: %s pointer not aligned to 32B",
                   "GXInitTexPreLoadRegion", "size even");
    ASSERTMSGLINEV(0x524, (size_odd & 0x1F) == 0,
                   "%s: %s pointer not aligned to 32B",
                      ...
                      GXTlutSize tlut_size)
{
    ...
    ASSERTMSGLINEV(0x583, (tmem_addr & 0x1FF) == 0,
                   "%s: tmem pointer is not aligned to 512B",
                   "GXInitTlutRegion");
    ASSERTMSGLINEV(0x584, tlut_size <= 0x400, "%s: tlut size exceeds 16K",
                   "GXInitTlutRegion");
    t->loadTlut1 = 0;
    tmem_addr -= 0x80000;
...
void GXPreLoadEntireTexture(GXTexObj* tex_obj, GXTexRegion* region)
{
    ...
    if (isMipMap != 0) {
        ...
        for (i = 0; i < maxLevelIndex; i++) {
            ...
            __GetImageTileCount(t->fmt, wd >> (i + 1), ht >> (i + 1),
                                &rowTiles, &colTiles, &cmpTiles);
    ...
    if (is32bit) {
        ...
        ASSERTMSGLINE(0x66E, totalEven <= r->sizeEven,
                      "GXPreLoadEntireTexture: Even tmem size does not match "
                      "the texture size");
        ASSERTMSGLINE(0x66F, totalEven <= r->sizeOdd,
                      "GXPreLoadEntireTexture: Odd tmem size does not match "
                      "the texture size");
    } else if (isMipMap != 0) {
        if (r->sizeEven > r->sizeOdd) {
            ASSERTMSGLINE(0x674, totalEven <= r->sizeEven,
                          "GXPreLoadEntireTexture: Even tmem size does not "
                          "match the texture size");
            ASSERTMSGLINE(0x675, totalOdd <= r->sizeOdd,
                          "GXPreLoadEntireTexture: Odd tmem size does not "
                          "match the texture size");
        } else {
    ...
    if (maxLevelIndex != 0) {
        ...
        for (i = 0; i < maxLevelIndex; i++) {
            ...
            __GetImageTileCount(t->fmt, (u16) (wd >> (i + 1)),
                                (u16) (ht >> (i + 1)), &rowTiles, &colTiles,
                                &cmpTiles);
...
void GXSetTexCoordScaleManually(GXTexCoordID coord, u8 enable, u16 ss, u16 ts)
{
    ...
    ASSERTMSGLINEV(0x6D3, coord < 8, "%s: bad texcoord specified",
...
void GXSetTexCoordCylWrap(GXTexCoordID coord, u8 s_enable, u8 t_enable)
{
    ...
    ASSERTMSGLINEV(0x6F5, coord < 8, "%s: bad texcoord specified",
...
void GXSetTexCoordBias(GXTexCoordID coord, u8 s_enable, u8 t_enable)
{
    ...
    ASSERTMSGLINEV(0x714, coord < 8, "%s: bad texcoord specified",
                   "GXSetTexCoordBias");
