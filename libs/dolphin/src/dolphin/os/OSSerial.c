...
static int __SITransfer(long chan, void* output, unsigned long outputBytes,
                        void* input, unsigned long inputBytes,
                        void (*callback)(long, unsigned long,
                                         struct OSContext*));
                                         ...
                                         struct OSContext*))
{
    ...
    ASSERTMSGLINE(
        0x12C, (outputBytes != 0) && (outputBytes <= 128),
        "SITransfer(): output size is out of range (must be 1 to 128).");
    ASSERTMSGLINE(
        0x12E, (inputBytes != 0) && (inputBytes <= 128),
...
unsigned long SISetXY(unsigned long x, unsigned long y)
{
    ...
    ASSERTMSGLINE(0x1CA, x >= 8,
                  "SISetXY(): x is out of range (8 <= x <= 255).");
    ASSERTMSGLINE(0x1CB, x <= 255,
                  "SISetXY(): x is out of range (8 <= x <= 255).");
    ASSERTMSGLINE(0x1CC, y <= 255,
                  "SISetXY(): y is out of range (0 <= y <= 255).");
...
static void AlarmHandler(struct OSAlarm* alarm, struct OSContext* context)
{
    ...
    ASSERTLINE(0x267, packet->time <=
                          OSGetTime()); // WTF? Dereferencing a NULL POINTER?
    packet = &Packet[chan];
