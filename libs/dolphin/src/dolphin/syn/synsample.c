...
static void __SYNSetupAdpcm(struct SYNVOICE* voice)
{
    ...
    if ((voice->region->loopStart + voice->region->loopLength) != 0) {
        ...
        // the hell? why not just write the struct members??? what is this
        // doing??? why not just write to the members directly?
        ...
        *(p) = *(adpcm);
        ...
        sampleStart = voice->synth->aramBaseNibble + voice->sample->offset;
        sampleEnd =
            sampleStart + __SYNGetNibbleAddress(voice->sample->length - 1);
