 ...
 * vertex descriptor is used by the Graphics Processor (GP) to interpret the
 ...
 * (frac), if required. The are eight vertex formats available in the VAT.
 * The vertex format describes the format of all attributes in a vertex.
 * The application can pre-program all eight vertex formats, and then select
 * one of them during the actual drawing of geometry (See GXBegin). Note
 * that all vertices used to draw a particular graphics primitive will have
 * the same format. The vertex format set using GXSetVtxAttrFmt, along with
 * the current vertex descriptor set using GXSetVtxDesc, completely define
 ...
 * the frac for these attributes is fixed according to the type. The component
 * count (count) for GX_VA_NRM must be set to GX_NRM_XYZ. The component count
 * for GX_VA_NBT must be set to GX_NRM_NBT (see exception for HW2 below).
 *
 ...
 * attributes (see GXSetVtxDesc), attr_idx is supplied in the vertex data.
 ...
 *
 * There is a base pointer, base_ptr, for each type of attribute as well as
 ...
 * The vertex cache fetches 32B of data for each cache miss; therefore,
 * there is a small performance benefit to aligning attribute arrays to
 * 32B, and possibly for arranging vertex data so that it doesn't span
 * 32B boundaries. Conveniently enough, OSAlloc returns 32-byte aligned
 * pointers. For static data arrays, you can use the ATTRIBUTE_ALIGN(32)
 * macro (specific to the MetroWerks compiler) to align the base_ptr to
 * 32B, as in:
 *
 ...
 * The generated texture coordinates are linked to specific textures and
 * specific Texture Environment (TEV) stages using GXSetTevOrder.
 *
