...
void ftDonkey_SpecialLw_StartMotion(HSD_GObj* fighter_gobj)
{
    Fighter* fp = GET_FIGHTER(fighter_gobj);
    fp->sv.dk.speciallw.x0 = 0;
    Fighter_ChangeMotionState(fighter_gobj, 0x17F, 0, NULL, 0.0f, 1.0f, 0.0f);
...
void ftDonkey_8010DE88(HSD_GObj* fighter_gobj)
{
    ...
    Fighter_ChangeMotionState(fighter_gobj, 0x180, 0, NULL, 0.0f, 1.0f, 0.0f);
...
{
    Fighter_ChangeMotionState(fighter_gobj, 0x181, 0, NULL, 0.0f, 1.0f, 0.0f);
}
...
void ftDonkey_8010E0CC(HSD_GObj* fighter_gobj)
{
    Fighter* fp = GET_FIGHTER(fighter_gobj);
    if (!fp->x2219_flag.bits.b0) {
        efSync_Spawn(0x4C6, fighter_gobj, fp->x5E8_fighterBones[1].x0_jobj);
...
void ftDonkey_8010E148(HSD_GObj* fighter_gobj)
{
    Fighter* fp = GET_FIGHTER(fighter_gobj);
    if (!fp->x2219_flag.bits.b0) {
        efSync_Spawn(0x4C7, fighter_gobj, fp->x5E8_fighterBones[1].x0_jobj);
        fp->x2219_flag.bits.b0 = 1;
    }
    fp->cb.x21D4_callback_EnterHitlag = efLib_PauseAll;
    fp->cb.x21D8_callback_ExitHitlag = efLib_ResumeAll;
