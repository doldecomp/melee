...
void ftDonkey_SpecialN_StartAction(HSD_GObj* fighter_gobj)
{
    ...
    if (fp->ev.dk.x222C == donkey_attr->SpecialN.x2C_MAX_ARM_SWINGS) {
        ...
        fp->x234C_stateVar4 = fp->ev.dk.x222C;
        fp->ev.dk.x222C = 0;
    } else {
...
void ftDonkey_8010E7B4(HSD_GObj* fighter_gobj)
{
    if (!ftAnim_IsFramesRemaining(fighter_gobj)) {
        Fighter_ActionStateChange_800693AC(fighter_gobj, 0x172, 0, NULL, 0.0f,
                                           1.0f, 0.0f);
        ftDonkey_SetCallbacks_SpecialN(fighter_gobj);
    }
}
...
{
    if (!ftAnim_IsFramesRemaining(fighter_gobj)) {
        Fighter_ActionStateChange_800693AC(fighter_gobj, 0x177, 0, NULL, 0.0f,
                                           1.0f, 0.0f);
        ftDonkey_SetCallbacks_SpecialN(fighter_gobj);
    }
}
    ...
    if (!func_8009917C(fighter_gobj)) {
        if ((fp->input.x668 & 0x200)) {
            Fighter_ActionStateChange_800693AC(fighter_gobj, 0x174, 0, NULL,
                                               0.0f, 1.0f, 0.0f);
            fp->x234C_stateVar4_s32 = fp->ev.dk.x222C;
            fp->ev.dk.x222C = 0;
            ftDonkey_SetCallbacks_SpecialN(fighter_gobj);
            func_8006EBA4(fighter_gobj);
        }
