...
void ftFox_SpecialHi_StartAction(HSD_GObj* fighter_gobj)
{
    ...
    fp->x2340_stateVar1 = (s32)foxAttrs->x54_FOX_FIREFOX_GRAVITY_DELAY;
...
{
    Fighter* fp = getFighter(fighter_gobj);
    func_8007592C(fp, func_8007500C(fp, 2), DOUBLE_PI32 - fp->x2344_f32);
}
...
void ftFox_SpecialHi_Phys(HSD_GObj* fighter_gobj)
{
    ...
    if ((s32)fp->x234C_stateVar4 >= foxAttrs->x70_FOX_FIREFOX_DURATION_END)
    {
...
void ftFox_SpecialAirHi_Phys(HSD_GObj* fighter_gobj)
{
    ...
    if ((s32)fp->x234C_stateVar4 >= foxAttrs->x70_FOX_FIREFOX_DURATION_END)
    {
        fp->x80_self_vel.x = (f32) -((fp->x2C_facing_direction * (foxAttrs->x78_FOX_FIREFOX_REVERSE_ACCEL * cosf(fp->x2344_f32))) - fp->x80_self_vel.x);
        fp->x80_self_vel.y = (f32) -((foxAttrs->x78_FOX_FIREFOX_REVERSE_ACCEL * sinf(fp->x2344_f32)) - fp->x80_self_vel.y);
    }
}
...
void ftFox_SpecialHi_Coll(HSD_GObj* fighter_gobj)
{
    ...
    {
        fp->x2344_f32 = func_someCalcAngle_80022C30(-collData->x154_groundNormal.x * fp->x2C_facing_direction, collData->x154_groundNormal.y);
...
inline BOOL ftFox_SpecialHi_IsBound(HSD_GObj* fighter_gobj)
{
    ...
    if (!(fp->x2350_stateVar5_s32 < foxAttrs->x6C_FOX_FIREFOX_BOUNCE_VAR))
    {
        return TRUE;
    }
...
void ftFox_SpecialAirHi_Coll(HSD_GObj* fighter_gobj)
{
    ...
    if (func_80081298(fighter_gobj) == FALSE) // Not a single line of this entire function makes the slightest sliver of sense but it matches so whatever :D
    {
        ...
        facingDir:
            ...
            fp->x2344_f32 = func_someCalcAngle_80022C30(fp->x80_self_vel.y, fp->x80_self_vel.x * fp->x2C_facing_direction);
            ftFox_SpecialHi_RotateModel(fighter_gobj);
        }
    }
}
...
void ftFox_SpecialAirHi_AirToGround(HSD_GObj* fighter_gobj)
{
    ...
    if (!((stick_x + stick_y) < foxAttrs->x64_FOX_FIREFOX_DIRECTION_STICK_RANGE_MIN))
    {
        ...
        if (!(lbvector_AngleXY(&collData->x154_groundNormal, &sp20) < HALF_PI32) && (func_8009A134(fighter_gobj) == FALSE))
        {
            ...
            fp->x2223_flag.bits.b4 = 1;
            fp->x2348_stateVar3 = (u32)(s32)tempAttrs->x68_FOX_FIREFOX_DURATION;
            fp->x234C_stateVar4 = 0.0f;
            fp->x2350_stateVar5 = 0.0f;
...
void ftFox_SpecialAirHi_Action(HSD_GObj* fighter_gobj)
{
    ...
    if ((stick_x + stick_y) >= foxAttrs->x64_FOX_FIREFOX_DIRECTION_STICK_RANGE_MIN)
    {
        ...
        fp->x2344_f32 = func_someCalcAngle_80022C30(fp->input.x624_lstick_y, fp->input.x620_lstick_x * fp->x2C_facing_direction);
    }
    else
    {
        fp->x2344_f32 = HALF_PI32;
    }
    ...
    fp->x2350_stateVar5 = 0;

    fp->x80_self_vel.x = fp->x2C_facing_direction * (foxAttrs->x74_FOX_FIREFOX_SPEED * cosf(fp->x2344_f32));
    fp->x80_self_vel.y = foxAttrs->x74_FOX_FIREFOX_SPEED * sinf(fp->x2344_f32);
    ftFox_SpecialHi_RotateModel(fighter_gobj);
    fp->cb.x21F8_callback = func_8007F76C;
    fp->cb.x21BC_callback_Accessory4 = ftFox_SpecialHi_CreateLaunchGFX;
...
inline void ftFox_SpecialHiBound_SetVars(HSD_GObj* fighter_gobj)
{
    ...
    {
        sp1C = -func_someCalcAngle_80022C30(collData->x154_groundNormal.x, collData->x154_groundNormal.y);
    }
    else
    {
        sp1C = 0.0f;
