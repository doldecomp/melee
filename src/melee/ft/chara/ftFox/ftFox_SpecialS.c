...
void ftFox_SpecialS_CreateGFX(HSD_GObj* fighter_gobj)
{
    ...
    if (fp->x2219_flag.bits.b0 == false) {
        efSync_Spawn(0x48D, fighter_gobj, fp->x5E8_fighterBones[0].x0_jobj,
                     &fp->facing_dir);
...
{
    return getFighter(fighter_gobj)->sv.fx.SpecialS.blendFrames[index];
}

// 0x800E9EE8
// https://decomp.me/scratch/7sYVM // Fox & Falco's grounded Illusion/Phantasm
// Start Action State handler
void ftFox_SpecialS_StartMotion(HSD_GObj* fighter_gobj)
{
    Fighter* fp = GET_FIGHTER(fighter_gobj);
    ...
    Fighter_ChangeMotionState(fighter_gobj, AS_FOX_SPECIALS_START, 0, NULL,
                              0.0f, 1.0f, 0.0f);
...
// Start Action State handler
void ftFox_SpecialAirS_StartMotion(HSD_GObj* fighter_gobj)
{
    Fighter* fp = GET_FIGHTER(fighter_gobj);
    ftFoxAttributes* foxAttrs = getFtSpecialAttrs(fp);
    ...
    Fighter_ChangeMotionState(fighter_gobj, AS_FOX_SPECIALAIRS_START, 0, NULL,
                              0.0f, 1.0f, 0.0f);
    ftAnim_8006EBA4(fighter_gobj);
...
void ftFox_SpecialAirSStart_Coll(HSD_GObj* fighter_gobj)
{
    ...
    if (ft_CheckGroundAndLedge(fighter_gobj, cliffCatchDir) != false) {
...
void ftFox_SpecialSStart_GroundToAir(HSD_GObj* fighter_gobj)
{
    ...
    Fighter_ChangeMotionState(fighter_gobj, AS_FOX_SPECIALAIRS_START,
...
void ftFox_SpecialAirSStart_AirToGround(HSD_GObj* fighter_gobj)
{
    ...
    Fighter_ChangeMotionState(fighter_gobj, AS_FOX_SPECIALS_START,
                              FTFOX_SPECIALS_COLL_FLAG, NULL,
                              fp->x894_currentAnimFrame, 1.0f, 0.0f);
}
...
static inline void ftFox_SpecialS_CreateGhostItem(HSD_GObj* fighter_gobj)
{
    ...
    if ((u32) fp->x2208_ftcmd_var2 == 1U) {
        ...
        if ((s32) fp->x4_fighterKind == FTKIND_FOX) {
            ghostGObj =
                it_8029CEB4(fighter_gobj, &fp->xB0_pos, 0x38, fp->facing_dir);
        } else {
            ghostGObj =
                it_8029CEB4(fighter_gobj, &fp->xB0_pos, 0x39, fp->facing_dir);
        }
        if (ghostGObj != NULL) {
...
static inline void ftFox_SpecialS_SetPhys(HSD_GObj* fighter_gobj)
{
    ...
    fp->sv.fx.SpecialS.ghostEffectPos[3] = fp->sv.fx.SpecialS.ghostEffectPos[2];
    fp->sv.fx.SpecialS.ghostEffectPos[2] = fp->sv.fx.SpecialS.ghostEffectPos[1];
    fp->sv.fx.SpecialS.ghostEffectPos[1] = fp->sv.fx.SpecialS.ghostEffectPos[0];
...
void ftFox_SpecialS_Action(HSD_GObj* fighter_gobj)
{
    ...
    Fighter_ChangeMotionState(fighter_gobj, AS_FOX_SPECIALS,
...
void ftFox_SpecialAirS_Action(HSD_GObj* fighter_gobj)
{
    ...
    Fighter_ChangeMotionState(fighter_gobj, AS_FOX_SPECIALAIRS,
                              FtStateChange_SkipUpdateRumble, NULL, 0.0f, 1.0f,
                              0.0f);
    ftFox_SpecialS_SetVars(fighter_gobj);
}
...
// 0x800EA944
// https://decomp.me/scratch/YLYPn // Fox & Falco's aerial Illusion/Phantasm End
// Animation callback
...
// 0x800EAA8C
// https://decomp.me/scratch/qGaVf // Fox & Falco's aerial Illusion/Phantasm End
// Physics callback
...
// 0x800EABCC
// https://decomp.me/scratch/pd3IU // Fox & Falco's aerial Illusion/Phantasm End
// Collision callback
void ftFox_SpecialAirSEnd_Coll(HSD_GObj* fighter_gobj)
{
    ...
    if (ft_CheckGroundAndLedge(fighter_gobj, cliffCatchDir) != false) {
        ft_800D5CB0(fighter_gobj, 0, foxAttrs->x50_FOX_ILLUSION_LANDING_LAG);
        return;
    }
    if (ftCliffCommon_80081298(fighter_gobj))
        return;
...
// 0x800EAC50
// https://decomp.me/scratch/ZZTJW // Fox & Falco's grounded Illusion End Action
// State handler
void ftFox_SpecialSEnd_Action(HSD_GObj* fighter_gobj)
{
    ...
    Fighter_ChangeMotionState(fighter_gobj, AS_FOX_SPECIALS_END,
                              FtStateChange_SkipUpdateRumble, NULL, 0.0f, 1.0f,
                              0.0f);
...
void ftFox_SpecialAirSEnd_Action(HSD_GObj* fighter_gobj)
{
    ...
    Fighter_ChangeMotionState(fighter_gobj, AS_FOX_SPECIALAIRS_END,
                              FtStateChange_SkipUpdateRumble, NULL, 0.0f, 1.0f,
                              0.0f);
    ftFox_SpecialSEnd_SetVars(fighter_gobj);
}
