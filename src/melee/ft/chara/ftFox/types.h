...
typedef struct ftFoxSpecialS {
    s32 gravityDelay; // 0x2340 - Number of frames to pass before gravity takes
                      // effect
    Vec3 ghostEffectPos[4]; // 0x2344 through 0x2370 - Related to
                            // Illusion/Phantasm model's position?
...
typedef struct ftFoxSpecialHi {
    ...
    s32 travelFrames; // 0x2348 - Number of frames Firefox/Firebird's launch
                      // animation lasts for
    s32 unk;          // 0x234C - ???
    s32 unk2;         // 0x2350 - ???
...
typedef struct ftFoxSpecialLw {
    s32 releaseLag; // 0x2340 - Auto lag frames after initializing Reflector if
                    // B is not being held. Reflector is immediately released
