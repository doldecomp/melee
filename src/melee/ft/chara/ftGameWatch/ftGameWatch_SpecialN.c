...
/* https://decomp.me/scratch/P7Z9j - first function of SpecialN.c; terrifying
   code https://decomp.me/scratch/kFkx6 - closer match, but just what is going
   on in this one? temp_fp suggests there is an inline somewhere in the
   function, but an inline depth of 2 causes the compiler to start using branch
   link; no combination of inlines / lack thereof I have tried fully match */
            ...
            gawAttrs->x1C_GAMEWATCH_CHEF_MAX)
        {
            ...
            for (i -= 5; i < 5; i++) {
                if ((i != temp_fp->sa.gaw.x2240_chefVar1) &&
                    (i != temp_fp->sa.gaw.x2244_chefVar2))
                {
