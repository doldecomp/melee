...
void ftGameWatch_ItemJudgementSetup(HSD_GObj* fighter_gobj)
{
    ...
    if (GET_FIGHTER(fighter_gobj)->x2204_ftcmd_var1 != 0U) {
        ...
        func_8000B1CC(fp->x5E8_fighterBones[func_8007500C(fp, 0x31)].x0_jobj,
                      NULL, &sp20);
        fp->ev.gaw.x2264_judgementGObj =
            func_802C7774(fp->facing_dir, fighter_gobj, &sp20,
                          func_8007500C(fp, 0x31), fp->ev.gaw.x222C_judgeVar1);
        if ((s32) fp->ev.gaw.x222C_judgeVar1 == 6) {
            sp14.z = 0.0f;
            sp14.x = 0.0f;
            sp14.y = 5.0f;
            func_8000B1CC(
        ...
        if (fp->ev.gaw.x2264_judgementGObj != NULL) {
...
void ftGameWatch_ItemJudgementRemove(HSD_GObj* fighter_gobj)
{
    ...
    if (fp->ev.gaw.x2264_judgementGObj != NULL) {
        func_802C7A84(fp->ev.gaw.x2264_judgementGObj);
...
static void ftGameWatch_ItemJudgementEnterHitlag(HSD_GObj* fighter_gobj)
{
    Fighter* fp = GET_FIGHTER(fighter_gobj);
    if (fp->ev.gaw.x2264_judgementGObj != NULL)
        func_802C7AD0(fp->ev.gaw.x2264_judgementGObj);
}
...
static void ftGameWatch_ItemJudgementExitHitlag(HSD_GObj* fighter_gobj)
{
    Fighter* fp = GET_FIGHTER(fighter_gobj);
    if (fp->ev.gaw.x2264_judgementGObj != NULL)
        func_802C7AF0(fp->ev.gaw.x2264_judgementGObj);
}
...
int ftGameWatch_SpecialS_GetRandomInt(HSD_GObj* fighter_gobj)
{
...

    {
        ...
        for (judge -= judge_max, i = 0, rand = 0; judge < judge_max; judge++) {
            if (judge != fp->ev.gaw.x222C_judgeVar1 &&
                judge != fp->ev.gaw.x2230_judgeVar2)
            {
...
#pragma clang diagnostic pop
        ...
        fp->ev.gaw.x2230_judgeVar2 = fp->ev.gaw.x222C_judgeVar1;
        fp->ev.gaw.x222C_judgeVar1 = result;
        return result;
    }
}
...
void ftGameWatch_SpecialS_StartAction(HSD_GObj* fighter_gobj)
{
    ...
    Fighter_ActionStateChange_800693AC(
        fighter_gobj, fp->ev.gaw.x222C_judgeVar1 + AS_GAMEWATCH_SPECIALS1, 0,
        NULL, 0.0f, 1.0f, 0.0f);
...
void ftGameWatch_SpecialAirS_StartAction(HSD_GObj* fighter_gobj)
{
    ...
    ftGameWatch_SpecialS_GetRandomInt(fighter_gobj);
    Fighter_ActionStateChange_800693AC(
        fighter_gobj, fp->ev.gaw.x222C_judgeVar1 + AS_GAMEWATCH_SPECIALAIRS1, 0,
        NULL, 0.0f, 1.0f, 0.0f);
    func_8006EBA4(fighter_gobj);
    ftGameWatch_SpecialS_SetVars(fighter_gobj);
}
...
static inline void ftGameWatch_SpecialS_SetCall(HSD_GObj* fighter_gobj)
{
    Fighter* fp = GET_FIGHTER(fighter_gobj);
...
// 0x8014CA88
// https://decomp.me/scratch/FPdQK // Mr. Game & Watch's ground -> air Judgement
// Acion State handler
static void ftGameWatch_SpecialS_GroundToAir(HSD_GObj* fighter_gobj)
{
    ...
    !fighter_gobj;
    Fighter_ActionStateChange_800693AC(
        fighter_gobj, fp->ev.gaw.x222C_judgeVar1 + AS_GAMEWATCH_SPECIALAIRS1,
...
static void ftGameWatch_SpecialAirS_AirToGround(HSD_GObj* fighter_gobj)
{
    ...
    !fighter_gobj;
    Fighter_ActionStateChange_800693AC(
        fighter_gobj, fp->ev.gaw.x222C_judgeVar1 + AS_GAMEWATCH_SPECIALS1,
        FTGAMEWATCH_SPECIALS_COLL_FLAG, NULL, fp->x894_currentAnimFrame, 1.0f,
        0.0f);
    ftGameWatch_SpecialS_SetCall(fighter_gobj);
}
