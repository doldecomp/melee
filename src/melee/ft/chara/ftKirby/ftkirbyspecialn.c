...
void ftKb_SpecialHi_800F3570(Fighter_GObj* gobj)
{
    ...
    if (0.0f != fp->mv.kb.specialhi.x18.x) {
        ...
        var_f3 = fp->mv.kb.specialhi.x18.x > 0.0f ? 1.0f : -1.0f;
        bottom_y = pos_y - slide_speed;
        pos_x = fp->cur_pos.x;
        if (mpCheckAllRemap(
                NULL, 0, NULL, &sp1C, -1, -1, pos_x, top_y,
                (fp->mv.kb.specialhi.x18.y * slide_speed * var_f3) + pos_x,
                bottom_y) == 0)
        {
            sp1C = p->vec;
        }
...
void ftKb_SpecialHi_800F37EC(Fighter_GObj* gobj)
{
    ...
    for (i = 3; i > 0; i--) {
        ...
        {
            for (j = i; j >= 0; j--) {
                if ((temp_f5 = fp->mv.kb.speciallw.x88[j]) < 0.0f) {
                    temp_f4 = -temp_f5;
                } else {
                    temp_f4 = temp_f5;
                }
                if (temp_f4 <= dat_attr->speciallw_min_slant_angle_slide) {
