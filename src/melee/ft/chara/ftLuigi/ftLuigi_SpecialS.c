...
// 0x80142A5C
// https://decomp.me/scratch/EHQ0r // Luigi's Green Missile Setup (RNG +
// calculations)
...
// 0x80142B14
// https://decomp.me/scratch/6eTeE // Luigi's grounded Green Missile Action
// State handler
...
// 0x80142B88
// https://decomp.me/scratch/Wb3jT // Luigi's aerial Green Missile Action State
// handler
...
// 0x80142C00
// https://decomp.me/scratch/tyVNQ // Luigi's Green Missile OnGiveDamage
// callback
...
// 0x80142C40
// https://decomp.me/scratch/Jk0de // Luigi's grounded Green Missile Start
// Animation callback
...
// 0x80142CC0
// https://decomp.me/scratch/AbUbj // Luigi's grounded Green Missile Start
// Physics callback
void ftLuigi_SpecialSStart_Phys(HSD_GObj* fighter_gobj)
{
    Fighter* fp = GET_FIGHTER(fighter_gobj);

    func_8007C930(fp, ((ftLuigiAttributes*) getFtSpecialAttrs(fp))
                          ->x1C_LUIGI_GREENMISSILE_UNK2);
    func_8007CB74(fighter_gobj);
}

// 0x80142D00
// https://decomp.me/scratch/O4crQ // Luigi's aerial Green Missile Start Physics
// callback
void ftLuigi_SpecialAirSStart_Phys(HSD_GObj* fighter_gobj)
{
    ...
    if (fp->x2200_ftcmd_var0 != 0) {
        func_8007D494(fp, luigiAttrs->x20_LUIGI_GREENMISSILE_FALLING_SPEED,
                      ftAttr->x170_TerminalVelocity);
    }
...
// 0x80142DD8
// https://decomp.me/scratch/MX0y2 // Luigi's Green Missile Start ground -> air
...
void ftLuigi_SpecialSStart_GroundToAir(HSD_GObj* fighter_gobj)
{
    ...
    func_8007D5D4(fp);
    Fighter_ActionStateChange_800693AC(fighter_gobj, AS_LUIGI_SPECIALAIRS_START,
                                       FTLUIGI_SPECIALS_COLL_FLAG, NULL,
                                       fp->x894_currentAnimFrame, 1.0f, 0.0f);
}

// 0x80142E38
// https://decomp.me/scratch/KJRhZ // Luigi's Green Missile Start air -> ground
// Action State handler
...
// 0x80142E98
// https://decomp.me/scratch/JZWFY // Luigi's grounded Green Missile Charge
// Animation callback
...
// 0x80142F38
// https://decomp.me/scratch/taZqq // Luigi's aerial Green Missile Charge
// Animation callback
...
// 0x80142FD8
// https://decomp.me/scratch/spI7k // Luigi's grounded Green Missile Charge IASA
// callback
void ftLuigi_SpecialSHold_IASA(HSD_GObj* fighter_gobj)
{
    if (!(GET_FIGHTER(fighter_gobj)->input.x65C_heldInputs & HSD_BUTTON_B))
        ftLuigi_SpecialSLaunch_Action(fighter_gobj);
}
...
// 0x801430F0
// https://decomp.me/scratch/IJjXT // Luigi's Green Missile Charge ground -> air
// Acion State handler
void ftLuigi_SpecialSHold_GroundToAir(HSD_GObj* fighter_gobj)
{
    ...
    Fighter_ActionStateChange_800693AC(
        fighter_gobj, AS_LUIGI_SPECIALAIRS_HOLD,
        (FIGHTER_GFX_PRESERVE | FTLUIGI_SPECIALS_COLL_FLAG), NULL,
        fp->x894_currentAnimFrame, 1.0f, 0.0f);
}
...
void ftLuigi_SpecialAirSHold_AirToGround(HSD_GObj* fighter_gobj)
{
    ...
    func_8007D7FC(fp);
    Fighter_ActionStateChange_800693AC(
        fighter_gobj, AS_LUIGI_SPECIALS_HOLD,
        (FIGHTER_GFX_PRESERVE | FTLUIGI_SPECIALS_COLL_FLAG), NULL,
        fp->x894_currentAnimFrame, 1.0f, 0.0f);
}

// 0x801431B0
// https://decomp.me/scratch/0FOkW // Luigi's grounded Green Missile Charge
// Action State handler
void ftLuigi_SpecialSHold_Action(HSD_GObj* fighter_gobj)
{
    ...
    Fighter_ActionStateChange_800693AC(fighter_gobj, AS_LUIGI_SPECIALS_HOLD,
                                       FIGHTER_SFX_PRESERVE, NULL, 0.0f, 1.0f,
                                       0.0f);
    GET_FIGHTER(fighter_gobj)->cb.x21BC_callback_Accessory4 =
        ftLuigi_SpecialS_SetGFX;
}

// 0x80143204
// https://decomp.me/scratch/0FOkW // Luigi's aerial Green Missile Charge Action
// State handler
...
// 0x80143258
// https://decomp.me/scratch/8478K // Luigi's grounded Green Missile Launch
// Animation callback
void ftLuigi_SpecialSLaunch_Anim(HSD_GObj* fighter_gobj)
{
    ...
    if (!fp->luigiVars[0].SpecialS.isMisfire &&
        fp->x914[0].tangibility == Invincible)
    {
...
// 0x80143304
// https://decomp.me/scratch/hlU8P // Luigi's aerial Green Missile Launch
// Animation callback
...
// 0x80143468
// https://decomp.me/scratch/IJjXT // Luigi's Green Missile Launch ground -> air
// Acion State handler
void ftLuigi_SpecialSLaunch_GroundToAir(HSD_GObj* fighter_gobj)
{
    ...
    Fighter_ActionStateChange_800693AC(fighter_gobj,
                                       ...
                                       FTLUIGI_SPECIALS_GROUND_AIR_FLAG, NULL,
                                       fp->x894_currentAnimFrame, 1.0f, 0.0f);
}
...
// 0x80143528
// https://decomp.me/scratch/EzvGh // Luigi's grounded Green Missile Launch
// Action State handler
...
// 0x80143600
// https://decomp.me/scratch/ehhGj // Luigi's aerial Green Missile Launch Action
// State handler
void ftLuigi_SpecialAirSLaunch_Action(HSD_GObj* fighter_gobj)
{
    ...
    Fighter_ActionStateChange_800693AC(
        fighter_gobj, AS_LUIGI_SPECIALAIRS_LAUNCH, 0, NULL, 0.0f, 1.0f, 0.0f);
    ftLuigi_SpecialS_RemoveGFX(fighter_gobj);
    ftLuigi_SpecialS_Setup(fighter_gobj);
}

// 0x801436D8
// https://decomp.me/scratch/04NUV // Luigi's grounded Green Missile Misfire
// Animation callback
...
// 0x80143784
// https://decomp.me/scratch/gBi6d // Luigi's aerial Green Missile Misfire
// Animation callback
...
// 0x801438E8
// https://decomp.me/scratch/Kx8l7 // Luigi's Green Missile Misfire ground ->
// air Action State handler
...
// 0x80143948
// https://decomp.me/scratch/2R5bm // Luigi's Green Missile Misfire air ->
// ground Action State Handler
...
// 0x801439A8
// https://decomp.me/scratch/EDozx // Luigi's grounded Green Missile Misfire
// Action State handler
...
void ftLuigi_SpecialAirSMisfire_Action(HSD_GObj* fighter_gobj)
{
    ...
    Fighter_ActionStateChange_800693AC(
        fighter_gobj, AS_LUIGI_SPECIALAIRS_MISFIRE, 0, NULL, 0.0f, 1.0f, 0.0f);
    ftLuigi_SpecialS_RemoveGFX(fighter_gobj);
    ftLuigi_SpecialS_Setup(fighter_gobj);
}
...
// 0x80143B64
// https://decomp.me/scratch/urKKQ // Luigi's aerial Green Missile Fly Physics
// callback
void ftLuigi_SpecialAirSFly_Phys(HSD_GObj* fighter_gobj)
{
    ...
    if (fp->x2200_ftcmd_var0 != 0) {
        func_8007D494(fp, luigiAttrs->x40_LUIGI_GREENMISSILE_GRAVITY_MUL,
                      luigiAttrs->x34_LUIGI_GREENMISSILE_GRAVITY_START);
    } else {
        func_8007D494(fp, luigiAttrs->x30_LUIGI_GREENMISSILE_MUL_Y,
                      luigiAttrs->x34_LUIGI_GREENMISSILE_GRAVITY_START);
    }
    if (fp->x2200_ftcmd_var0 != 0)
...
// 0x80143BE4
// https://decomp.me/scratch/bjBo1 // Luigi's aerial Green Missile Fly Collision
// callback
...
// 0x80143DFC
// https://decomp.me/scratch/GZ27s // Luigi's grounded Green Missile End Physics
// callback
...
// 0x80143E3C
// https://decomp.me/scratch/9y6Ua // Luigi's aerial Green Missile End Physics
// callback
...
// 0x80143F18
// https://decomp.me/scratch/zZpxK // Luigi's Green Missile End air -> ground
// Action State handler
...
// 0x80143F70
// https://decomp.me/scratch/pVzMc // Luigi's Green Missile End ground -> air
// Action State handler
