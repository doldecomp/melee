...
static void ftMh_MS_341_8014FE5C(HSD_GObj* gobj)
{
    ...
    if (fp->motion_id == ftMh_MS_Unk341 || fp->motion_id == ftMh_MS_Unk342) {
        fp->cur_pos = fp->fv.mh.x2240_pos;
    } else {
        fp->fv.mh.x2240_pos = fp->cur_pos;
    }
    if (fp->fv.mh.x2258 == ftMh_MS_Unk389) {
        Fighter_ChangeMotionState(gobj, ftMh_MS_Unk341, 0, 0,
                                  fp->x894_currentAnimFrame, 1, 0);
    } else {
        Fighter_ChangeMotionState(gobj, ftMh_MS_Unk341, 0, 0, 0, 1, 0);
    }
...
void ftMh_MS_341_8014FF1C(HSD_GObj* gobj)
{
    ...
    if (fp->fv.mh.x2258 == ftMh_MS_Unk390) {
        Fighter_ChangeMotionState(gobj, ftMh_MS_Unk342, 0, 0,
                                  fp->x894_currentAnimFrame, 1, 0);
    } else {
        Fighter_ChangeMotionState(gobj, ftMh_MS_Unk342, 0, 0, 0, 1, 0);
    }
    fp->fv.mh.x2258 = ftMh_MS_Unk342;
}
...
struct MasterHandDataStuff ftMh_Init_803D40D0 = {
    { ftMh_MS_Sweep_StartMotion, ftMh_MS_Slap_StartMotion,
      ftMh_MS_Walk_StartMotion, ftMh_MS_Punch_StartMotion,
      ftMh_MS_GroundSlap_StartMotion, ftMh_MS_Poke_StartMotion,
      ftMh_MS_Drill_StartMotion, ftMh_MS_Ram_StartMotion,
      ftMh_MS_Crush_StartMotion, ftMh_MS_Jet_StartMotion,
      ftMh_MS_Lasers_StartMotion, ftMh_MS_Gun_StartMotion,
      ftMh_MS_Grab_Unk0_MotionStart, ftMh_MS_Grab_Unk1_MotionStart },
...
void ftMh_MS_341_Anim(HSD_GObj* gobj)
{
    ...
    if (Player_GetPlayerSlotType(fp->xC_playerID) == 0) {
        ...
        if (!ftAnim_IsFramesRemaining(gobj)) {
            if (fp->motion_id == ftMh_MS_Unk341) {
                doAnim0(gobj);
            } else if (fp->motion_id == ftMh_MS_Unk342) {
                doAnim1(gobj);
            }
        }
    } else {
        if (--fp->fv.mh.x223C < 0) {
            ...
            if (ftBossLib_8015C44C(28) == ftMh_MS_Unk384) {
                ...
                case ftMh_MS_Lasers_StartMotion: // Lasers
                    ...
                    break;
                case ftMh_MS_Gun_StartMotion: // Gun
                    ftMh_MS_363_801530A4(gobj);
                    break;
                case ftMh_MS_Grab_Unk0_MotionStart: // Grab
                    ftMh_MS_372_801542E0(gobj);
                    break;
                case ftMh_MS_Grab_Unk1_MotionStart: // Grab
                    ftMh_MS_372_801542E0(gobj);
                    break;
                }
            }
...
void ftMh_MS_341_80150894(HSD_GObj* gobj)
{
    ...
    if (l_pressed && (buttons & HSD_BUTTON_DPAD_UP)) {
        ...
        // Grab
        fp->fv.mh.x2250 = ftMh_MS_Grab_Unk0_MotionStart;
        ftMh_MS_372_801542E0(gobj);
    } else if ((buttons & HSD_BUTTON_Z) && (buttons & HSD_BUTTON_DPAD_RIGHT)) {
        // Grab
        fp->fv.mh.x2250 = ftMh_MS_Grab_Unk1_MotionStart;
        ftMh_MS_372_801542E0(gobj);
