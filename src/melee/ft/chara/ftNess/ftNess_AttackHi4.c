...
void ftNess_YoyoUpdateHitPos(HSD_GObj* fighter_gobj)
{
    ...
    if (hitbox_data->state != HitCapsule_Disabled) {
        if (fp->ev.ness.x2230_yoyoHitboxPos.x != 0.0f ||
            fp->ev.ness.x2230_yoyoHitboxPos.y != 0.0f)
        {
            func_8007B8A8(&fp->x914[0], &fp->ev.ness.x2230_yoyoHitboxPos);
        }
    }
};
...
static void ftNess_YoyoApplyDamage(f32 unk_float, HSD_GObj* fighter_gobj)
{
    Fighter* fp = fighter_gobj->user_data;
...
void ftNess_YoyoSetHitPos(HSD_GObj* fighter_gobj)
{
    Vec3 sp2C;
    Vec3 sp20;
    Vec3 sp14;
    ...
    fp->ev.ness.x2230_yoyoHitboxPos = sp2C;
}
...
void ftNess_YoyoSetHitPosUnk(HSD_GObj* fighter_gobj, f32 pos_unk)
{
    ...
    sp30 = fp->ev.ness.x2230_yoyoHitboxPos;
    pos_update = 1.0f - pos_unk;
    fp->ev.ness.x2230_yoyoHitboxPos.x =
        (f32) ((sp3C.x * pos_unk) + (sp30.x * pos_update));
    fp->ev.ness.x2230_yoyoHitboxPos.y =
        (f32) ((sp3C.y * pos_unk) + (sp30.y * pos_update));
    fp->ev.ness.x2230_yoyoHitboxPos.z =
...
bool ftNess_YoyoCheckNoObstruct(HSD_GObj* fighter_gobj)
{
    ...
    sp20.y += fp->xB0_pos.y;
    sp20.z += fp->xB0_pos.z;
    sp14 = fp->ev.ness.x2230_yoyoHitboxPos;

    if (!ftNess_YoyoCheckEnvColl(fighter_gobj, &sp20, &sp14, ECB_MUL_Y)) {
        sp14 = fp->ev.ness.x2230_yoyoHitboxPos;
        sp20 = sp14;
...
void ftNess_YoyoSetVarAll(HSD_GObj* fighter_gobj)
{
    ...
    fp->ev.ness.x2230_yoyoHitboxPos.z = 0.0f;
    fp->ev.ness.x2230_yoyoHitboxPos.y = 0.0f;
    fp->ev.ness.x2230_yoyoHitboxPos.x = 0.0f;
    fp->ev.ness.x223C = 0.0f;
}
...
void ftNess_YoyoApplySmash(HSD_GObj* fighter_gobj)
{
    HSD_GObj* yoyo_GObj;
    ...
    temp_yoyo = fp->ev.ness.x222C_yoyoGObj;
    yoyo_GObj = temp_yoyo;
...
void ftNess_YoyoSetChargeDamage(HSD_GObj* fighter_gobj)
{
    Item* item_data;
    Article* article;
    Fighter* fp;
    ...
    yoyo_GObj = fp->ev.ness.x222C_yoyoGObj;
    ...
    if (yoyo_GObj) {
        ...
        fp->ev.ness.x223C = yoyo_attr->x18_SPIN_TEXANIM_SPEED;
    }
...
void ftNess_YoyoSetUnkRate(HSD_GObj* fighter_gobj)
{
    ...
    if (yoyo_GObj != NULL) {
        ...
        fp->ev.ness.x223C = texanim_unk - yoyo_float;
    }
}

void ftNess_YoyoCreateItem(HSD_GObj* fighter_gobj)
{
    Vec3 sp10;
    Fighter* fp = GET_FIGHTER(fighter_gobj);

    func_8000B1CC(fp->x5E8_fighterBones[0x2A].x0_jobj, NULL, &sp10);
    fp->ev.ness.x222C_yoyoGObj =
        func_802BE9D8(fp->facing_dir, fighter_gobj, &sp10, fp->action_id);
    fp->x1984_heldItemSpec = fp->ev.ness.x222C_yoyoGObj;
    if (fp->ev.ness.x222C_yoyoGObj != NULL) {
...
void ftNess_YoyoItemSetFlag(HSD_GObj* fighter_gobj)
{
    Fighter* fp = GET_FIGHTER(fighter_gobj);

    ftNess_YoyoItemSetUnk2(fighter_gobj);
    fp->ev.ness.x222C_yoyoGObj = NULL;
}
...
void ftNess_YoyoItemSetUnk(HSD_GObj* fighter_gobj)
{
    Fighter* fp = GET_FIGHTER(fighter_gobj);
    if (fp->ev.ness.x222C_yoyoGObj != NULL) {
        func_802BE598(fp->ev.ness.x222C_yoyoGObj);
    }
}
...
void ftNess_YoyoItemSetUnk2(HSD_GObj* fighter_gobj)
{
    ...
    if (fp->ev.ness.x222C_yoyoGObj)
        func_802BE5B8(fp->ev.ness.x222C_yoyoGObj);
}
...
void ftNess_AttackHi4_Action(HSD_GObj* fighter_gobj)
{
...

    {
        ...
        fp->nessVars.AttackHi4.isPosUpdateMod = 1;
        fp->ev.ness.x2230_yoyoHitboxPos.z = 0.0f;
        fp->ev.ness.x2230_yoyoHitboxPos.y = 0.0f;
        fp->ev.ness.x2230_yoyoHitboxPos.x = 0.0f;
        fp->ev.ness.x223C = 0.0f;
...
void ftNess_YoyoStartTimedRehit(HSD_GObj* fighter_gobj)
{
    Fighter* fp;
    ftNessAttributes* ness_attr;
...
void ftNess_AttackHi4_Anim(HSD_GObj* fighter_gobj)
{
    Vec3 sp24;
    Vec3 sp18;
    Fighter* fighter_data2;
    ...
    if (ftNess_YoyoThink_IsRemove(fighter_gobj) == false) {
            ...
            ((s32) fighter_data2->nessVars.AttackHi4.isChargeDisable == false))
        {
            ...
            sp24 = fighter_data2->ev.ness.x2230_yoyoHitboxPos;
            if ((ftNess_YoyoCheckEnvColl(fighter_gobj, &sp18, &sp24, 0.5f) ==
                 0) &&
                (sp24 = fighter_data2->ev.ness.x2230_yoyoHitboxPos, sp18 = sp24,
    ...
    HSD_GObj* fighter_gobj) // Ness's Up Smash Physics callback
{
    ...
    fp->ev.ness.x2230_yoyoHitboxPos = sp10;
}

static inline HSD_GObj* GetFighterData_x222C(
    HSD_GObj* fighter_gobj) // Yet another inline needed to match
{
    Fighter* fp = fighter_gobj->user_data;
    HSD_GObj* yoyo_GObj = fp->ev.ness.x222C_yoyoGObj;
    return yoyo_GObj;
}
...
{
    return fp->ev.ness.x222C_yoyoGObj;
};
    ...
    HSD_GObj* fighter_gobj) // Ness's Up Smash Charge Animation callback
{
        ...
        NULL)
    {
        ...
        temp_fp->ev.ness.x223C = yoyo_attr->x20_UNK_TEXANIM_MOD - unk_float;
    }
    ...
    HSD_GObj* fighter_gobj) // Ness's Up Smash Post-Charge Physics callback
{
    ...
    if (yoyoSmashFrameCurr < 0x18) {
        ...
        sp30 = fighter_data2->ev.ness.x2230_yoyoHitboxPos;
        temp_f2 = 1.0f - phi_f31;
        fighter_data2->ev.ness.x2230_yoyoHitboxPos.x =
            (f32) ((sp24.x * phi_f31) + (sp30.x * temp_f2));
        fighter_data2->ev.ness.x2230_yoyoHitboxPos.y =
            (f32) ((sp24.y * phi_f31) + (sp30.y * temp_f2));
        fighter_data2->ev.ness.x2230_yoyoHitboxPos.z =
            (f32) ((sp24.z * phi_f31) + (sp30.z * temp_f2));
        return;
    }
    fighter_data3 = GET_FIGHTER(fighter_gobj);
    ftNess_YoyoSetUnkPos(fighter_gobj, &sp18);
    fighter_data3->ev.ness.x2230_yoyoHitboxPos = sp18;
}
...
void ftNess_AttackHi4_Release_Action(HSD_GObj* fighter_gobj)
{
    Vec3 sp34;
    Fighter* fighter_data2;
    Fighter* fp;
    ...
    if ((yoyo_GObj = fighter_data2->ev.ness.x222C_yoyoGObj) != NULL) {
        yoyo_attr = GetYoyoAttr(yoyo_GObj);
        sp34 = YoyoChargePos;
        func_802C0010(yoyo_GObj, &sp34);
        fighter_data2->ev.ness.x223C = yoyo_attr->x18_SPIN_TEXANIM_SPEED;
    }
