    ...
    HSD_GObj* gobj) // Ness's F-Smash Action State handler
{
    ...
    fp->fv.ns.x2248_baseballBatGObj = baseballBatGObj;
    ...
    HSD_GObj* gobj) // Check if Ness is in F-Smash + has Baseball Bat item
{
    ...
    if (fp->fv.ns.x2248_baseballBatGObj == NULL) {
        return true;
    }
...
void ftNess_ItemNessBatRemove(HSD_GObj* gobj) // Remove Baseball Bat
{
    ...
    if (fp->fv.ns.x2248_baseballBatGObj != NULL) {
        func_802AD6B8(fp->fv.ns.x2248_baseballBatGObj);
...
void ftNess_AttackS4_Anim(HSD_GObj* gobj) // Ness's F-Smash Animation callback
{
            ...
            gobj)) // Check if animation has frames remaining
    {
        ...
        if (fighter_data2->fv.ns.x2248_baseballBatGObj != NULL) {
            func_802AD6B8(
                fighter_data2->fv.ns
                    .x2248_baseballBatGObj); // Despawn the Baseball Bat if
                                             // animation is over
            fighter_data2->fv.ns.x2248_baseballBatGObj = NULL;
        }
        func_8008A2BC(gobj);
    }
}
