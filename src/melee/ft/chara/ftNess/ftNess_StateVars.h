...
typedef struct ftNessSpecialN {
    s32 flashTimerLoop1; // 0x2340 - Number of frames Ness remains in the
                         // "charge loop" animation after releasing PK Flash
    s32 flashTimerLoop2; // 0x2344 - Number of extra frames Ness remains in the
                         // "charge loop" animation after releasing PK Flash?
    s32 gravityDelay;  // 0x2348 - Number of frames to pass before gravity takes
                       // effect
    s32 flashTimerMin; // 0x234C - Minimum number of frames required to pass for
                       // PK Flash to auto-release if B is not being held

} ftNessSpecialN;

typedef struct ftNessSpecialHi {
    s32 thunderColl; // 0x2340 - PK Thunder collision type? Used to check if PK
                     // Thunder is colliding with Ness
    s32 thunderTimerLoop1; // 0x2344 - Number of frames Ness remains in the
                           // "control loop" animation after PK Thunder despawns
    s32 thunderTimerLoop2; // 0x2348 - Number of extra frames Ness remains in
                           // the "control loop" animation after PK Thunder
                           // despawns?
...
typedef struct ftNessSpecialLw {
    s32 releaseLag; // 0x2340 - Auto lag frames after initializing PSI Magnet if
