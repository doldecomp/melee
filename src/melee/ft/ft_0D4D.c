...
void ftCo_800D4E50(Fighter* fp, Vec3* arg1, int arg2, float arg8)
{
    if (gm_8016B0B4() && !fp->is_sub_fighter) {
        int coins = Player_GetCoins(fp->slot);
        int var_r31 = coins / 2;
        Player_SetCoins(fp->slot, coins - var_r31);
        Player_SetUnk9C(fp->slot, var_r31 + Player_GetUnk9C(fp->slot));
...
void ftCo_800D4FF4(Fighter_GObj* gobj)
{
    ...
    Fighter_Spawn(gobj);
    fp->smash_attrs.x2135 = Player_80032F30(fp->slot);

    if (fp->smash_attrs.x2135 == -1) {
        Player_GetSpawnPlatformPos(fp->slot, &sp44);
        ...
        Player_GetSomePos(fp->slot, &sp20);
        fp->mv.co.turn.facing_after =
...
void ftCo_Rebirth_Phys(Fighter_GObj* gobj)
{
    ...
    if (!new_var->is_sub_fighter) {
        if (new_var->smash_attrs.x2135 != -1) {
            ...
            Player_GetSomePos(new_var->slot, &player_pos);
        ...
        HSD_GObj* other_gobj = Player_GetEntityAtIndex(new_var->slot, 0);
...
void fn_800D54A4(Fighter_GObj* gobj)
{
    ...
    u8 _[8];
    Fighter* fp = gobj->user_data;
    HSD_GObj* other_gobj = Player_GetEntityAtIndex(fp->slot, 1);
...
void fn_800D55B4(Fighter_GObj* gobj)
{
    Fighter* fp = gobj->user_data;
    Fighter* new_var;
    HSD_GObj* other_gobj = Player_GetEntityAtIndex(fp->slot, 0);
    Fighter* other_fp = other_gobj->user_data;
    new_var = other_fp;
...
void ftCo_RebirthWait_IASA(Fighter_GObj* gobj)
{
    ...
    if (!fp->is_sub_fighter) {
        HSD_GObj* companion = Player_GetEntityAtIndex(fp->slot, 1);
        if (companion != NULL &&
        ...
        if (ftLib_800873CC(Player_GetEntityAtIndex(fp->slot, 0)) != 0) {
            return;
        }
        var_r30 = 1;
    }
    ...
    ftColl_8007B7A4(gobj, p_ftCommonData->x5D8);
    pl_80040374(fp->slot, fp->is_sub_fighter);
}
...
void ftCo_RebirthWait_Phys(Fighter_GObj* gobj)
{
    ...
    if (!new_var->is_sub_fighter) {
        if (new_var->smash_attrs.x2135 != -1) {
            ...
            Player_GetSomePos(new_var->slot, &sp24);
            new_var->mv.co.common.x4.x =
                new_var->facing_dir * ftCommon_800804EC(new_var) +
                (sp18.x + sp24.x);
        ...
        Fighter* other_fp =
            Player_GetEntityAtIndex(new_var->slot, 0)->user_data;
        new_var->self_vel.x = other_fp->self_vel.x;
        new_var->self_vel.y = other_fp->self_vel.y;
    }
}
