...
void ftColl_80076CBC(Fighter* fp0, HitCapsule* hit0, Fighter* fp1)
{
...

    {
        ...
        if (!fp1->x221C_b2) {
            ...
            fp1->x19BC_shieldDamageTaken3 = fp0->slot;
            ...
            pl_8003E150(fp1->slot, fp1->is_sub_fighter);
                     ...
                     f32 val)
{
    ...
    if (ftLib_80086960(item->owner)) {
        ...
        fp->x19BC_shieldDamageTaken3 = owner_fp->slot;
                     ...
                     HitCapsule* hit2)
{
    if (item->xCC_item_attr->x0_78 == 4) {
        ...
        if (!fp->stamina_dead) {
            ...
            pl_8003E17C(fp->slot, fp->is_sub_fighter, item->entity);
        }
        return false;
    }
...
{
    return i + fp->slot * 2 + fp->is_sub_fighter;
}
                     ...
                     int arg8)
{
    ...
    if (attacker != NULL) {
        ...
        victim->dmg.x18c4_source_ply = attacker->slot;
        victim->dmg.x18C8 = -1;
...
void ftColl_800787B4(Item_GObj* arg0, Fighter_GObj* arg1, int arg2)
{
    ...
    if (ip->kind == It_Kind_BombHei) {
        pl_80041B08(fp->slot, (UNK_T) (uintptr_t) fp->is_sub_fighter,
                    (u16) ip->x1C);
    }
...
void ftColl_8007891C(Fighter_GObj* arg0, Fighter_GObj* arg1, float arg2)
{
    ...
    fp0 = arg0->user_data;
    fp1 = arg1->user_data;
    pl_8003EB30(arg2, fp0->slot, fp0->is_sub_fighter, fp1->slot,
...
void ftColl_80078998(HSD_GObj* arg0, HSD_GObj* arg1, float arg2)
{
    ...
    if (ftLib_80086960(ip->owner)) {
        ...
        pl_8003EB30(arg2, owner_fp->slot, owner_fp->is_sub_fighter,
                    victim_fp->slot, victim_fp->is_sub_fighter,
                    ip->xD90.x2073);
    }
}
#ifdef MUST_MATCH
             ...
             victim_gobj != NULL; victim_gobj = victim_gobj->next)
        {
            if (this_gobj == victim_gobj) {
                ...
                if ((victim_fp->x1064_thrownHitbox.owner != NULL)) {
                    if (this_fp->slot != victim_fp->grabber_unk1) {
                        flag1 = true;
                    } else {
                        continue;
                    }
                }
                if (flag1 || (this_fp->slot != victim_fp->slot)) {
                      ...
                      int unk_count)
{
    ...
    float defense = Player_GetDefenseRatio(fp->slot);
    float attack = Player_GetAttackRatio(attacker->slot);
    ...
    for (i = 0, entry = entries; i < idx; entry++, i++) {
        ...
        case 1: {
            ...
            kb = ftColl_80079AB0(fp, hit, unk_count, gm_8016B248(),
                                 Player_GetAttackRatio(attacker_fp->slot),
                                 Player_GetDefenseRatio(fp->slot), co->weight);
        ...
        case 2: {
            ...
            if (ftLib_80086960(tail_owner_gobj)) {
                ...
                attack = Player_GetAttackRatio(owner_fp->slot);
            } else {
                attack = 1.0F;
            }

            kb = co->weight;
            defense = Player_GetDefenseRatio(fp->slot);
            stage = gm_8016B248();
            hit = entry->hit0;
            ftd = p_ftCommonData;
            w = kb;
...
void ftColl_8007B8CC(Fighter* fp, Fighter_GObj* grabber_gobj)
{
    ...
    fp->grabber_unk1 = grabber_fp->slot;
}
...

    {
        ...
        case HSD_GOBJ_CLASS_FIGHTER:
            fighter_victim = fp->gobj;
            {
                ...
                pl_8003EB30(dmg_amount, src_fp->slot, src_fp->is_sub_fighter,
                            victim_fp->slot, victim_fp->is_sub_fighter,
                            src_fp->x2070.x2073);
            }
            break;
            ...
            item_victim = fp->gobj;
            {
                ...
                ip = s2->user_data;
                {
                    ...
                    if (ftLib_80086960(tail_owner_gobj)) {
                        ...
                        pl_8003EB30(dmg_amount, owner_fp->slot,
                                    owner_fp->is_sub_fighter, victim_fp->slot,
