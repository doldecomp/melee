...
bool func_8008169C(HSD_GObj* pPlayerEntityStruct /*r3*/)
{
    ...
    if (fp_r31->x2224_flag.bits.b7) {
            ...
            (pCollData_r6->x134_envFlags & MPCOLL_FLAGS_B5))
        {
            ...
            if ((fp_r31->x210C_walljumpInputTimer <
                 MAX_WALLJUMP_INPUT_FRAMES) &&
                (wallSide_f31 == fp_r31->x2110_walljumpWallSide))
            {
                ...
                if (wallSideFlag_r0) {
                    ...
                    if (!func_800567C0(pCollData_r6->x174_leftwall.index,
                                       &ecb_sp18, &wallPos_sp24))
                        wallPos_sp24.x = 0.0f;
                } else {
                    ...
                    ecb_sp18.z += fp_r31->xB0_pos.z;
                    // compute distance to the wall?
                    if (!func_800567C0(pCollData_r6->x160_rightwall.index,
                                       &ecb_sp18, &wallPos_sp24))
                        wallPos_sp24.x = 0.0f;
                }
                // not sure what this computes, I guess it checks if we are
                // close to the wall and move towards it with sufficent speed
            ...
            if ((f32) fp_r31->x210C_walljumpInputTimer <
                    p_ftcommon_r4->x768 && // walljump timer within limits?
                ((fp_r31->x2110_walljumpWallSide == -1.0f &&
                  fp_r31->input.x620_lstick_x >=
                      p_ftcommon_r4
                          ->x76C) || // left wall & control stick right?
                 (fp_r31->x2110_walljumpWallSide == 1.0f &&
                  fp_r31->input.x620_lstick_x <=
                      -p_ftcommon_r4->x76C) // right wal & control stick left?
                 ) &&
                (f32) fp_r31->x670_timer_lstick_tilt_x <
                    p_ftcommon_r4->x770 // control stick didn't stay too long in
                                        // the tilt area?
            )
            {
                // do a walljump!
                func_800C1E64(pPlayerEntityStruct, 0xCB, p_ftcommon_r4->x774,
                              fp_r31->x1969_walljumpUsed,
                              fp_r31->x2110_walljumpWallSide);
