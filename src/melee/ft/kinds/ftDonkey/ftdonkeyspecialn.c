...
void ftDk_SpecialN_Enter(HSD_GObj* gobj)
{
    ...
    if (fp->u.dk.x222C == donkey_attr->SpecialN.x2C_MAX_ARM_SWINGS) {
        Fighter_ChangeMotionState(gobj, ftDk_MS_SpecialNFull, Ft_MF_None, 0, 1,
                                  0, NULL);
        fp->mv.dk.specialn.x8 = 1;
        fp->mv.dk.specialn.xC = fp->u.dk.x222C;
        fp->u.dk.x222C = 0;
    } else {
        Fighter_ChangeMotionState(gobj, ftDk_MS_SpecialNStart, Ft_MF_None, 0,
                                  1, 0, NULL);
...
void ftDk_SpecialAirN_Enter(HSD_GObj* gobj)
{
    ...
    u8 _[4];
    if (fp->u.dk.x222C == donkey_attr->SpecialN.x2C_MAX_ARM_SWINGS) {
        Fighter_ChangeMotionState(gobj, ftDk_MS_SpecialAirNFull, Ft_MF_None, 0,
                                  1, 0, NULL);
        fp->mv.dk.specialn.x8 = 1;
        fp->mv.dk.specialn.xC = fp->u.dk.x222C;
        fp->u.dk.x222C = 0;
    } else {
        Fighter_ChangeMotionState(gobj, ftDk_MS_SpecialAirNStart, Ft_MF_None,
                                  0, 1, 0, NULL);
        fp->mv.dk.specialn.x8 = 0;
        fp->mv.dk.specialn.xC = 0;
    }
...
void ftDk_SpecialNStart_Anim(HSD_GObj* gobj)
{
    if (!ftAnim_IsFramesRemaining(gobj)) {
        Fighter_ChangeMotionState(gobj, ftDk_MS_SpecialNLoop, Ft_MF_None, 0, 1,
                                  0, NULL);
        setCallbacks(gobj);
    }
}
...
{
    if (!ftAnim_IsFramesRemaining(gobj)) {
        Fighter_ChangeMotionState(gobj, ftDk_MS_SpecialAirNLoop, Ft_MF_None, 0,
                                  1, 0, NULL);
        setCallbacks(gobj);
    }
}
...
void ftDk_SpecialNLoop_IASA(HSD_GObj* gobj)
{
    ...
    if (!ftCo_8009917C(gobj)) {
        if ((fp->input.pressed_buttons & HSD_PAD_B)) {
            Fighter_ChangeMotionState(gobj, ftDk_MS_SpecialN, Ft_MF_None, 0, 1,
                                      0, NULL);
            fp->mv.dk.specialn.xC = fp->u.dk.x222C;
            fp->u.dk.x222C = 0;
            setCallbacks(gobj);
        ...
        if (fp->cur_anim_frame == 0 && fp->mv.dk.specialn.x0) {
            Fighter_ChangeMotionState(gobj, ftDk_MS_SpecialNCancel, Ft_MF_None,
                                      0, 1, 0, NULL);
            setCallbacks(gobj);
        }
    }
...
void ftDk_SpecialAirNLoop_IASA(HSD_GObj* gobj)
{
    ...
    u8 _[4];
    if (fp->input.pressed_buttons & HSD_PAD_B) {
        Fighter_ChangeMotionState(gobj, ftDk_MS_SpecialAirN, Ft_MF_None, 0, 1,
                                  0, NULL);
        fp->mv.dk.specialn.xC = fp->u.dk.x222C;
        fp->u.dk.x222C = 0;
        setCallbacks(gobj);
    ...
    if (fp->cur_anim_frame == 0 && fp->mv.dk.specialn.x0) {
        Fighter_ChangeMotionState(gobj, ftDk_MS_SpecialAirNCancel, Ft_MF_None,
                                  0, 1, 0, NULL);
        setCallbacks(gobj);
    }
}
