...
bool ftNn_Init_80123954(Fighter_GObj* nana_gobj, GroundOrAir pp_ga)
{
    ...
    if (nana_gobj != NULL) {
        ...
        popo_fp = GET_FIGHTER(Player_GetEntityAtIndex(nana_fp->slot, 0));
        ...
        default: {
            ...
            if (dxsq + dysq < (s32) (popo_fp->x34_scale.y * (dd * dd))) {
                ...
                nana_fp->x1A5C = Player_GetEntityAtIndex(nana_fp->slot, 0);
            } else {
                nana_fp->x1A5C = NULL;
                ret = true;
            }
...
{
    Fighter* nana_fp = GET_FIGHTER(nana_gobj);
    Fighter* popo_fp = GET_FIGHTER(Player_GetEntityAtIndex(nana_fp->slot, 0));
    PAD_STACK(8);
    nana_fp->self_vel = popo_fp->self_vel;
