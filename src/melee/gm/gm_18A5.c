...
void fn_8018AA74(HSD_JObj* jobj, s32 entry_idx, s32 slot_idx)
{
    ...
    slot = BracketEntry_GetSlot(entry, slot_idx);
    ...
    HSD_JObjSetTranslateX(jobj, (f32) slot->x44);
    HSD_JObjSetTranslateY(jobj, -(f32) slot->x48);
}
...
void fn_8018B090(HSD_GObj* arg0)
{
    ...
    case 38:
        ...
        }
        {
            ...
            pick = sb + r * 0x2C;
            ((u8*) &lbl_80473AB8[sb[5]] + sb[6] * 0x2C)[0x50] =
                pick[0x50];
            ((u8*) &lbl_80473AB8[sb[5]] + sb[6] * 0x2C)[0x51] =
                pick[0x51];
            ((u8*) &lbl_80473AB8[sb[5]] + sb[6] * 0x2C)[0x52] =
                pick[0x52];
            ((u8*) &lbl_80473AB8[sb[5]] + sb[6] * 0x2C)[0x4D] =
                pick[0x4D];
            ((u8*) &lbl_80473AB8[sb[5]] + sb[6] * 0x2C)[0x4E] =
                pick[0x4E];
            ((u8*) &lbl_80473AB8[sb[5]] + sb[6] * 0x2C)[0x4F] =
                pick[0x4F];
            pick[0x30] = 0;
            pick[0x4E] = 3;
            tm->cur_option = 0x27;
...
void fn_80191FD4(HSD_GObj* gobj)
{
    ...
    if (lbl_803D9D20.x72[hud] != 0) {
        ...
        if ((s32) tm->x37[slot].x3 == fn_8018F6DC(fn_8018F3BC((s32) idx))) {
            fn_8019044C(sibling, (f32) ((tm->x37[slot].x7 * 0x1E) +
...
void fn_801935B8(void)
{
    ...
    for (i = 0; i < 6; i++) {
        ...
        (&tm->match_type)[i] = TmSettingTable_GetMin(i, 0);
    }
...
void fn_80194BC4(s32* arg0, u32 arg1, u32 arg2)
{
    ...
    if (arg1 & 0x40001) {
        ...
        arr_ptr = arg0 + option;
        value = *(++arr_ptr);
        if (value > TmSettingTable_GetMin(option, match_type)) {
            *arr_ptr = value - 1;
        } else {
            *arr_ptr = TmSettingTable_GetMax(option, match_type);
        }
    } else if (arg1 & 0x80002) {
        lbAudioAx_80024030(2);
        lbl_804799B8.x8 = 5;
        ...
        arr_ptr = arg0 + option;
        value = *(++arr_ptr);
        if (value < TmSettingTable_GetMax(option, match_type)) {
            *arr_ptr = value + 1;
        } else {
            *arr_ptr = TmSettingTable_GetMin(option, match_type);
        }
    }
    ...
    if (trigger & 0x100) {
        ...
        case 0xE:
            if (GetNameText(lbl_804799B8.x5 + (lbl_804799B8.x6 * 4)) != NULL) {
                idx = lbl_804799B8.x2 + lbl_804799B8.x3;
                tm_alt->x37[idx].x9 =
                    lbl_804799B8.x5 + (lbl_804799B8.x6 * 4);
            }
            *state_ptr = 0xF;
            fn_80190ABC(4);
            return;
    ...
    if ((trigger & 0x1100) != 0) {
        if (*state == 0x11) {
            ...
            if (gm_804771C4.match_type == 0) {
                ...
                base_val = i;
                while (i < td->x2E) {
                    td->x37[i].xE = td->x37[i].xD;
                    td->x37[i].xF = base_val;
                    if (i < td->x30) {
                        td->x4B8[i].x6 = td->x37[i].x9;
                        td->x4B8[i].x5 = td->x37[i].x2;
                        td->x4B8[i].x1 = td->x37[i].x3;
                        td->x4B8[i].x0 = td->x37[i].x0;
                        td->x4B8[i].x3 = td->x37[i].x7;
                        td->x4B8[i].x2 = td->x37[i].x5;
                    }
                    i++;
                }
...
{
    s32 rand;
    rand = HSD_Randi(lbl_80473AB8[arg0].x51 + lbl_80473AB8[arg0].x7D + lbl_80473AB8[arg0].xA9 + lbl_80473AB8[arg0].xD5);

    if (rand < (s32) lbl_80473AB8[arg0].x51) {
        lbl_80473AB8[arg0].x4C = 0;
        rand = HSD_Randi(lbl_80473AB8[arg0].x7D + lbl_80473AB8[arg0].xA9 + lbl_80473AB8[arg0].xD5);
    ...
    if (rand < (s32) (lbl_80473AB8[arg0].x7D + lbl_80473AB8[arg0].x51)) {
        ...
        rand = HSD_Randi(lbl_80473AB8[arg0].x51 + lbl_80473AB8[arg0].xA9 + lbl_80473AB8[arg0].xD5);
        if (rand < (s32) lbl_80473AB8[arg0].x51) {
    ...
    if (rand < (s32) (lbl_80473AB8[arg0].xA9 + lbl_80473AB8[arg0].x51 + lbl_80473AB8[arg0].x7D)) {
        lbl_80473AB8[arg0].xA4 = 0;
        rand = HSD_Randi(lbl_80473AB8[arg0].x51 + lbl_80473AB8[arg0].x7D + lbl_80473AB8[arg0].xD5);
        if (rand < (s32) lbl_80473AB8[arg0].x51) {
            lbl_80473AB8[arg0].x4C = 1;
            rand = HSD_Randi(lbl_80473AB8[arg0].x7D + lbl_80473AB8[arg0].xD5);
            if (rand < (s32) lbl_80473AB8[arg0].x7D) {
    ...
    rand = HSD_Randi(lbl_80473AB8[arg0].x51 + lbl_80473AB8[arg0].x7D + lbl_80473AB8[arg0].xA9);
    if (rand < (s32) lbl_80473AB8[arg0].x51) {
        lbl_80473AB8[arg0].x4C = 1;
        rand = HSD_Randi(lbl_80473AB8[arg0].x7D + lbl_80473AB8[arg0].xA9);
        if (rand < (s32) lbl_80473AB8[arg0].x7D) {
