...
{
    ResultsDisplayDataOverlay* disp = (&lbl_8046E1B0);
...
static inline void inline3(void)
{
    ResultsDisplayDataOverlay* disp = (&lbl_8046E1B0);
...
void fn_8017AA78(const u8* arg0)
{
    ...
    for (i = 0; i < 4; i++) {
        ...
        state->x6[i] = 0;
        state->score_tbl[i] = *(PackedS16x4*) &lbl_803D7038[i * 2];
        state->x22F4[i] = *(PackedS16x4*) &lbl_803D7018[i * 2];
    }
}
