...
/// Repeating per-slot unit of BracketEntry starting at x2C.
...
void fn_8019A158(void)
{
    ...
    if (mode == 1) {
        ...
        cursor = GET_BRACKET_SLOT(bracket, 0);
        for (i = 0; i < 4; i++) {
            if (i == slot) {
                GET_BRACKET_SLOT(bracket, slot)->x4C = 0;
            } else {
                cursor->x4C = 3;
            }
            cursor++;
        ...
        for (i = 0; i < 4; i++) {
            if (cursor->x4E == 3) {
                ...
                u8 v;
                u8* p = lbl_804799D8.x48 + i * 0xA8;
                v = p[0x5E];
                p[0x5D] = v;
                cursor->x4C = v;
                if (((MatchEnd*) lbl_804799D8.x48)
                        ->player_standings[i]
                        .is_small_loser == 0)
                {
                    sel = i;
...

        {
            ...
            ((MatchEnd*) lbl_804799D8.x48)->player_standings[0].is_big_loser =
                v;
            ((MatchEnd*) lbl_804799D8.x48)
                ->player_standings[0]
                .is_small_loser = v;
            v = lbl_80473AB8[bracket_idx].x78;
            ((MatchEnd*) lbl_804799D8.x48)->player_standings[1].is_big_loser =
                v;
            ((MatchEnd*) lbl_804799D8.x48)
                ->player_standings[1]
                .is_small_loser = v;
            v = lbl_80473AB8[bracket_idx].xA4;
            ((MatchEnd*) lbl_804799D8.x48)->player_standings[2].is_big_loser =
                v;
            ((MatchEnd*) lbl_804799D8.x48)
                ->player_standings[2]
                .is_small_loser = v;
            v = lbl_80473AB8[bracket_idx].xD0;
            ((MatchEnd*) lbl_804799D8.x48)->player_standings[3].is_big_loser =
                v;
            ((MatchEnd*) lbl_804799D8.x48)
                ->player_standings[3]
                .is_small_loser = v;
...

    {
        ...
        cursor = GET_BRACKET_SLOT(&lbl_80473AB8[bracket_idx], sel);
        {
            u8 model_idx = cursor->x50;
