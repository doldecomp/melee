...
Vec3 grI1_803B8268;
extern double const grI1_804DB5F8;
...
#define DOBJ_DIRECT_CLEAR_LOOP(jobj)                                          \
    {                                                                         \
        HSD_DObj* dobj = HSD_JObjGetDObj(jobj);                               \
        while (dobj != NULL) {                                                \
            HSD_DObjClearFlags(dobj, 1U);                                     \
...
typedef struct grInishie1_Data {
    char filename[0x10];
    struct block_table_struct block_table[BLOCK_COUNT];
    char report_fmt[0x10];
    char checkloop_assert[0x1C];
} grInishie1_Data;
...
grInishie1_Data grI1_803E49A8 = {
    ...
    "%s:%d: oioi..\n",
    "i!=HATENA_APPEAR_CHECKLOOP",
};

char grI1_803E4A30[] = "!mapgp->u.map.block[ix].hatena_gobj";
char grI1_803E4A54[] = "map_gobj";

static inline char* grInishie1_Filename(void)
{
    return grI1_803E49A8.filename;
}
...
HSD_GObj* grInishie1_801FA9B4(s32 arg0)
{
    ...
    if (gobj != NULL) {
        ...
        OSReport(grI1_803E4950.report_fmt_get_gobj, grInishie1_Filename(),
                 0xE7, arg0);
    }
...
void grInishie1_801FAC58(Ground_GObj* gobj)
{
    ...
    if (gp && jobj) {
        ...
        if (gp->gv.inishie12.xC4 != NULL) {
            {
                Vec3* vec_ptr = &vec;
                lb_8000B1CC(gp->gv.inishie12.xC4, NULL, vec_ptr);
            }
...
void grInishie1_801FAD84(HSD_GObj* gobj)
{
    ...
    block_table = grI1_803E49A8.block_table;
    ...
    while ((u32) i < BLOCK_COUNT) {
        ...
        jobj = Ground_801C3FA4(gobj, block_table[j].jobj_idx);
        if (jobj == NULL) {
            invalid_state(0x21D);
        }
...
typedef struct grInishie1_TargetBlock {
    s16 status;
    s16 x2;
    s16 x4;
    s16 x6;
    ...
    s16 x20;
    s16 x22;
} grInishie1_TargetBlock;

typedef struct grInishie1_TargetGround {
    u8 pad[0xC4];
    u16 xC4;
    s16 xC6;
    s16 xC8;
    ...
    s32 xD4;
    s32 xD8;
    grInishie1_TargetBlock* blocks;
} grInishie1_TargetGround;
    ...
    if (countdown > 0) {                                                      \
        ...
        if (countdown == 0) {                                                 \
            ...
            do {                                                              \
                ...
                if (index != last_index && gp->blocks[index].status == 0) {   \
                    break;                                                    \
                }                                                             \
                attempts++;                                                   \
            } while (attempts < 0x14);                                        \
            ...
            if (attempts == 0x14) {                                           \
                HSD_ASSERTMSG(line, false, "%s:%d: oioi..\n");            \
                                                                              ...
                                                                              \
            last_index = index;                                               \
            gp->blocks[index].status = status_val;                            \
            grInishie1_801FBAA0(gobj, index);                                 \
            gp->blocks[index].x20 = 0;                                        \
        }                                                                     \
    }

void grInishie1_801FB3F0(HSD_GObj* gobj)
{
    grInishie1_TargetGround* gp = gobj->user_data;
    u32 i;
    PAD_STACK(56);
          ...
          gp->xC6 < grI1_804D69F8->unk14)))
    {
        ...
        for (i = 0; i < 0x13; ++i) {
            gp->blocks[i].status = 3;
            grInishie1_801FBAA0(gobj, i);
            gp->blocks[i].x20 = 0;
        }
    } else {
        HATENA_APPEAR_CHECKLOOP(gp->xC6, gp->xCA, 1, 0x2D0U);
        HATENA_APPEAR_CHECKLOOP(gp->xC8, gp->xCC, 2, 0x2EBU);
    }

    for (i = 0; i < 0x13; ++i) {
        switch (gp->blocks[i].x2) {
        case 2:
            if (gp->blocks[i].x4 != 0) {
                gp->blocks[i].x4--;
            }
        ...
        case 3:
            gp->blocks[i].x10 = 1.2f;
            grInishie1_801FB0AC(gobj, i);
            gp->blocks[i].x2 = 4;
            // fallthrough

        case 4:
            gp->blocks[i].xC += gp->blocks[i].x10;

            if (gp->blocks[i].xC > 5.0) {
                gp->blocks[i].x10 = -1.2f;
            } else if (gp->blocks[i].xC < 0.0) {
                gp->blocks[i].xC = 0.0f;
                gp->blocks[i].x2 = 2;
                gp->blocks[i].x4 = 10;
            }
            ...
            // sure what this is
            HSD_JObjSetTranslateY(gp->blocks[i].jobj,
                                  gp->blocks[i].x8 + gp->blocks[i].xC);
            break;

        case 0:
            if (gp->blocks[i].x20 != 0) {
                gp->blocks[i].x20--;
            } else {
                Vec3 pos;
                lb_8000B1CC(gp->blocks[i].jobj, NULL, &pos);
                if (!grLib_801C9EE8(&pos, 15.0f)) {
                    DOBJ_CLEAR_LOOP(gp->blocks[i].jobj);
                    HSD_JObjClearFlagsAll(gp->blocks[i].jobj, JOBJ_HIDDEN);
                    grMaterial_801C8E08(gp->blocks[i].item_gobj);
                    gp->blocks[i].x2 = 1;
                    gp->blocks[i].x22 = grI1_804D69F8->unk18;
                }
            }

            if (gp->blocks[i].hatena_gobj != NULL) {
                HSD_JObj* sub_jobj =
                    ((HSD_GObj*) gp->blocks[i].hatena_gobj)->hsd_obj;
        ...
        case 1:
            ...
            */
            if (gp->blocks[i].x22 > 0) {
                HSD_JObjGetDObj(gp->blocks[i].jobj);
                if (gp->blocks[i].hatena_gobj != NULL) {
                    HSD_JObj* target =
                        ((HSD_GObj*) gp->blocks[i].hatena_gobj)->hsd_obj;

                    if (gp->blocks[i].x22 & 1) {
                    ...
                    if (gp->blocks[i].x22 & 1) {
                        DOBJ_DIRECT_CLEAR_LOOP(gp->blocks[i].jobj);
                    } else {
                        DOBJ_DIRECT_LOOP(gp->blocks[i].jobj);
                    }
                }
...
void grInishie1_801FBAA0(HSD_GObj* gobj, s32 index)
{
    ...
    if (hatena != NULL) {
        Ground* hatena_gp = hatena->user_data;
        if (gp->gv.inishie1.blocks[index].hatena_gobj) {
            __assert(grI1_803E49A8.filename, 0x39E, grI1_803E4A30);
        }
...
void grInishie1_801FBCEC(HSD_GObj* gobj, u32 index)
{
        ...
        gp->gv.inishie1.blocks[index].status == 3)
    {
        ...
        it_8026F7C8(&item_spawn_offset, &vec, 0);
    }
...
void grInishie1_801FC110(HSD_GObj* gobj)
{
...

    {
        ...
        if (zero == dist_lo && zero == dist_hi) {
            ...
            if (dist_lo != zero && dist_hi != zero &&
                gp->gv.inishie1.xF4 != zero)
            {
...
{
    struct block_table_struct* block_table = grI1_803E49A8.block_table;
    s32 count = BLOCK_COUNT;
    s32 idx = 0;
    while (count != 0) {
        if (block_id == block_table->idx) {
            return idx;
        }
        block_table += 1;
        idx += 1;
    ...
    OSReport(grI1_803E49A8.report_fmt, grI1_803E49A8.filename, 0x1F0);
    while (true) {
    }
    return idx;
}
                 ...
                 mpLib_GroundEnum arg4, f32 dist)
{
    ...
    if (fabsf_inline(dist) > grI1_804DB5F8) {
