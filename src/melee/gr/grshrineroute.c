        ...
        if (result != -1) {
            ...
            if (!(gp->gv.shrineroute.xC6 & (1 << ix))) {
                *(volatile u16*) &gp->gv.shrineroute.xC8 = (u16) result;
                if (gp->gv.shrineroute.symbols[(u32) ix] == NULL) {
                    __assert("grshrineroute.c", 0x213,
                             "gp->u.map.symbol[ix]");
                }
                {
                    s32 mid =
...
void grShrineRoute_80209AF0(Ground_GObj* gobj)
{
    ...
    gp->gv.shrineroute3.xCC = grSh_Route_804DB880;
    gp->gv.shrineroute3.xC8 = grSh_Route_804DB880;
    gp->gv.shrineroute3.xD0 = 0.00006981317F * HSD_Randf() + 0.000017453292F;
    gp->gv.shrineroute3.xD0 *=
        (HSD_Randi(2) != 0) ? grSh_Route_804DB888 : -1.0F;
...
void grShrineRoute_8020A8A4(Ground_GObj* gobj)
{
    ...
    if (gp->gv.shrineroute3.xC4 != NULL) {
        ...
        gp->gv.shrineroute3.xD8 =
            0.04363323f *
            ((2.0f * grShrineRoute_8020A8A4_rand()) - grSh_Route_804DB888);
