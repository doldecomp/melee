...
void it_802982AC(Item_GObj* item_gobj, Item_GObj* owner_gobj, Article* article) {
    it_80275158((HSD_GObj* ) item_gobj, ((ItLGunRayAttr*) ((Item*) item_gobj->user_data)->xC4_article_data->x4_specialAttributes)->x4);
...
int it_80298300(HSD_GObj* item_gobj) {
    ...
    item->x40_vel.x = item->xDD4_itemVar.lgunray.x8 * cosf(item->xDD4_itemVar.lgunray.x4);
    item->x40_vel.y = item->xDD4_itemVar.lgunray.x8 * sinf(item->xDD4_itemVar.lgunray.x4);
    item->x40_vel.z = 0.0f;
    ...
    if (item_jobj == NULL) {
        HSD_JObjSetRotationY(item_jobj, (1.5707963267948966 * item->facing_dir));
    // }
        ...
        // if (item_jobj == NULL) {
        HSD_JObjSetRotationX(item_jobj, (3.141592653589793 + atan2f(item->x40_vel.y, var_f2)));
        // }
...
int it_802985F8(HSD_GObj* item_gobj) {
    CollData item_coll = ((Item*) item_gobj->user_data)->x378_itemColl;  // Not sure how this is used
