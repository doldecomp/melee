#ifndef GALE01_2AF940
#define GALE01_2AF940

#include <platform.h>

#include "it/forward.h"

#include "it/items/types.h"

/* 2AF940 */ UNK_RET it_802AF940(UNK_PARAMS);
/* 2AFA2C */ void it_802AFA2C(Item_GObj*);
/* 2AFA70 */ UNK_RET it_802AFA70(UNK_PARAMS);
/* 2AFBA4 */ bool itNesspkflushexplode_UnkMotion_Anim(Item_GObj* gobj);
/* 2AFD2C */ void itNesspkflushexplode_UnkMotion_Phys(Item_GObj*);
/* 2AFD44 */ bool itNesspkflushexplode_UnkMotion_Coll(Item_GObj* gobj);
/* 2AFD4C */ bool it_802AFD4C(Item_GObj*);
/* 2AFD54 */ bool it_802AFD54(Item_GObj*);
/* 2AFD5C */ bool it_802AFD5C(Item_GObj*);
/* 2AFD64 */ bool it_802AFD64(Item_GObj*);
/* 2AFD6C */ void it_802AFD6C(Item_GObj*, Item_GObj*);
/* 3F6F40 */ extern ItemStateTable it_803F6F40[];

#endif
