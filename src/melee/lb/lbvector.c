...
extern float func_fatan2_80022C30(float y , float x);
...
Vec *lbvector_EulerAnglesFromONB(Vec *result_angles, Vec *a, Vec *b, Vec *c)
{
    if (b->z == -1.0f || b->z == 1.0f)
    {
        if (b->z == -1.0f)
        {
            ...
            result_angles->x = func_fatan2_80022C30(c->x, c->y);
        }
        else
        {
            result_angles->y = -1.5707963705062866f; // -pi/2
    ...
    else
    {
        ...
        result_angles->x = func_fatan2_80022C30(c->z, a->z);
        result_angles->z = func_fatan2_80022C30(b->y, b->x);
    }
