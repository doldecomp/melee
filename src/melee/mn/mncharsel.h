#ifndef GALE01_25BC20
#define GALE01_25BC20

#include <placeholder.h>

#include <baselib/forward.h>

/* 25BC20 */ TextKerning* mnCharSel_8025BC20(TextKerning* arg0, u32 arg1);
/* 25BD30 */ void mnCharSel_8025BD30(void);
/* 25C020 */ void mnCharSel_8025C020(int);
/* 25D1C4 */ UNK_RET mnCharSel_8025D1C4(UNK_PARAMS);
/* 25D5AC */ UNK_RET mnCharSel_8025D5AC(UNK_PARAMS);
/* 25DAA0 */ UNK_RET mnCharSel_8025DAA0(UNK_PARAMS);
/* 25DB34 */ UNK_RET mnCharSel_8025DB34(UNK_PARAMS);
/* 25EE8C */ UNK_RET mnCharSel_8025EE8C(UNK_PARAMS);
/* 25F0E0 */ UNK_RET fn_8025F0E0(UNK_PARAMS);
/* 25FAC0 */ UNK_RET fn_8025FAC0(UNK_PARAMS);
/* 25FB2C */ UNK_RET fn_8025FB2C(UNK_PARAMS);
/* 25FB50 */ UNK_RET mnCharSel_8025FB50(UNK_PARAMS);
/* 25FDEC */ UNK_RET mnCharSel_8025FDEC(UNK_PARAMS);
/* 260094 */ UNK_RET mnCharSel_CostumeChange(UNK_PARAMS);
/* 2602A0 */ void mnCharSel_CursorThink(HSD_GObj* gobj);
/* 262648 */ UNK_RET fn_80262648(UNK_PARAMS);
/* 262F44 */ UNK_RET fn_80262F44(UNK_PARAMS);
/* 263354 */ UNK_RET fn_80263354(UNK_PARAMS);
/* 2633B0 */ UNK_RET fn_802633B0(UNK_PARAMS);
/* 264070 */ UNK_RET mnCharSel_80264070(UNK_PARAMS);
/* 26407C */ UNK_RET fn_8026407C(UNK_PARAMS);
/* 2640A0 */ UNK_RET mnCharSel_802640A0(UNK_PARAMS);
/* 26688C */ void mnCharSel_8026688C_OnEnter(void*);
/* 2669F4 */ void mnCharSel_802669F4_OnFrame(void);
/* 266D70 */ void mnCharSel_80266D70_OnLeave(UNK_T);

#endif
