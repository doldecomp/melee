/** @file
 * @todo Rename to @c plUnk1.
 */
#ifndef MELEE_PL_PL_UNKNOWN_001_H
#define MELEE_PL_PL_UNKNOWN_001_H

#include <sysdolphin/baselib/gobj.h>

void func_80037C60(HSD_GObj*, s32);
void func_8003E058(s32, s32, s32, s32);
void func_8003EC9C(s32, s32, f32, f32);
void func_8003FAA8(s32 slot, s32 index, Vec3* pos, Vec3* prevPos);
void func_8003FE1C(u8 slot, bool);
void func_8003FFDC(u8, s32, u8, s32, s32);
void func_80040048(u8, s32);
void func_80040330(u8 slot, bool, f32);
void func_80040B8C(u8 slot, bool, s32);
void func_800411C4(u8 slot, bool);
void func_80041280(u8 slot, bool);
u32 func_800386D8(void*, s32);
void func_80040120(u8, s32);
void func_8003FC44(u8 slot, bool);
void func_800402D0(u8 slot, bool, s32);
void func_8003EA08(u8 slot, bool);
void func_80040460(u8 slot, bool);
void func_8003E4A4(u8 slot, bool, s32, s32);
void func_8003891C(int slot);
void func_80038F10(int slot);
void func_8003715C(void);
void func_80037590(void);
void func_8003E150(void);
void func_8003E17C(void);
void func_80038144(void);
void func_80078710(void);
void func_8003D60C(void);
void func_80041B08(void);
void func_8003EB30(void);
void func_8003722C(void);
void func_800373CC(void);
void func_80037ECC(void);

#endif
