...
void pl_80037C60(Fighter_GObj* fgp, s32 prev2070_int)
{
    ...
    fp = GET_FIGHTER(fgp);
    acp = Player_GetActionStats(fp->slot);
...
void pl_80037DF4(HSD_GObj* gobj, union Struct2070* ev)
{
    ...
    plActionStats* acp = Player_GetActionStats(ft->slot);
    local_ev = *ev;
    acp->attacks.total++;
                        ...
                        s32 arg3, s32 arg4)
{
    ...
    if (ifMagnify_802FB6E8(fp->slot) != 0) {
                 ...
                 s32 source_ply)
{
    ...
    if (attacker_fp != NULL && ev.x2073 != 0) {
        ...
        pl_800410F4(attacker_fp->slot, attacker_fp->is_sub_fighter,
                    victim_fp->slot, victim_fp->is_sub_fighter, ev.x2073);
    }
        ...
        victim_fp->dmg.x18ec_instancehitby != attack_instance)
    {
        *(s32*) &victim_fp->dmg.x18d4 = ev.x2070_int;
        ...
        if (attacker_fp != NULL && victim_fp->dmg.x18d4.x3 != 0) {
            ...
            {
                pl_80040D8C(attacker_fp->slot, attacker_fp->is_sub_fighter);
            }
            ...
            if (attack_id2 < 0x64) {
                ...
                acp = Player_GetActionStats(attacker_fp->slot);
                ev_reload.x2070_int = *(s32*) &victim_fp->dmg.x18d4;
                fp2 = GET_FIGHTER(attacker_gobj);
                acp2 = Player_GetActionStats(GET_FIGHTER(attacker_gobj)->slot);
                ...
                if (attack_id2 == 0x63) {
                    pl_8003FE40(fp2->slot, fp2->is_sub_fighter);
                }
                        ...
                        acp2->x358_hits.by_attack_counts[attack_id2])
                {
                    ...
                    pl_8003DFF4(fp2->slot, fp2->is_sub_fighter, attack_id2);
                }
                ...
                pl_8003FE64(attacker_fp->slot, attacker_fp->is_sub_fighter);
                pl_8003ED0C(attacker_fp->slot, attacker_fp->is_sub_fighter,
                            victim_fp->slot, victim_fp->is_sub_fighter,
                            victim_fp->dmg.x1830_percent);
                x18d4_x3 = ev_data->xC;
                pl_8003EA40(attacker_fp->slot, attacker_fp->is_sub_fighter,
                            victim_fp->slot, victim_fp->is_sub_fighter,
                            x18d4_x3);
                pl_800403FC(attacker_fp->slot, attacker_fp->is_sub_fighter,
                            victim_fp->slot, victim_fp->is_sub_fighter,
                            victim_fp->dmg.x18d4.x3);
                pl_80040FBC(attacker_fp->slot, attacker_fp->is_sub_fighter,
                            victim_fp->slot, victim_fp->is_sub_fighter,
                            victim_fp->dmg.x18d4.x3);
            }
        }
...
void pl_800384DC(HSD_GObj* gobj, int arg1, void* arg2)
{
    ...
    if (ev.x2073 != 0 && ev.x2073 < 0x64) {
        ...
        attack_id = ev2.x2073;
        if (attack_id == 0x63) {
            pl_8003FE40(fp->slot, fp->is_sub_fighter);
        }
                                    ...
                                    acp->x358_hits.by_attack_counts[attack_id])
        {
            ...
            pl_8003DFF4(fp->slot, fp->is_sub_fighter, attack_id);
        }
    }
