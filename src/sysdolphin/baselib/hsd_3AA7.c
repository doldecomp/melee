...
s32 fn_803B0E9C(struct CardState* arg0, s32 arg1, s32 arg2, s32 arg3, s32 arg4)
{
    ...
    if (arg3 == 0) {
        if (arg4 != 0) {
            for (block_idx = 0;
                 (u32) block_idx < (0x2F + state->x24 + state->x8) / state->x8;
                 block_idx++)
            {
                s32 cmd[9];
...
s32 fn_803B1338(CardState* state, s32 arg1)
{
        ...
        if (arg1 != 0) {
            ...
            if (fdata == NULL) {
                ...
                fn_803AC168(cmd);
                {
                    ...
                    cmd2[6] = (s32) fdata;
                    cmd2[7] = ofs0;
                    cmd2[8] = state->x8 - hdr_offset - 0x20;
                    result = fn_803AC168(cmd2);
                }
    ...
    for (file_id = 1; file_id < 9; file_id++) {
        ...
        if (file_blocks > 0) {
            ...
            } else if (state->x28[file_id] < 0 || state->x28[file_id] >= 4) {
                /* skip */
            } else {
                phys += file_blocks;
            }
