#ifndef SYSDOLPHIN_BASELIB_3B27_H
#define SYSDOLPHIN_BASELIB_3B27_H

#include <platform.h>
#include <placeholder.h>

/* 3B27F4 */ int hsd_803B27F4(s32*, const char*, int, int, void (*)(int, int));
/* 3B286C */ int hsd_803B286C(s32*, UNK_T, const char*, int, int, void (*)(int, int));
/* 3B2928 */ int hsd_803B2928(s32*, const char*, int, int, void (*)(int, int));
/* 3B29D8 */ int hsd_803B29D8(s32*, int, int, UNK_T);
/* 3B2A4C */ int hsd_803B2A4C(s32*, int, int, void (*)(int, int));
/* 3B2ADC */ void hsd_803B2ADC(s32*, UNK_T);

#endif
