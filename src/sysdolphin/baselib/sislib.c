    ...
    for (in_idx = 0; in_idx < 0x80; in_idx++, str_cursor++) {
             ...
             lut_idx++)
        {
                ...
                (sjis_lo == lut_ptr[lut_idx * 2 + 1]))
            {
                temp_idx = out_idx;
                out_idx += 1;
                data[temp_idx] = out_ptr[lut_idx * 2];
                temp_idx = out_idx;
                out_idx += 1;
                data[temp_idx] = out_ptr[lut_idx * 2 + 1];
                break;
            }
        }
    }
...
static inline void HSD_SisLib_EnsureTextBuffer(HSD_Text* text, s32 size)
{
    u16 old_size = text->x6E;

    if (old_size < (s32) (text->x6C + size)) {
        s32 new_size = old_size + 0x10;
        s32 count;
        s32 i;
        char* old_buffer = text->string_buffer;
        char* src;

        text->string_buffer = HSD_SisLib_Alloc(new_size);
        src = old_buffer;
        text->x6E = new_size;
        count = old_size;
        i = 0;
        while (i < count) {
            text->string_buffer[i++] = *src++;
        }
        i = count;
        while (i < text->x6E) {
            text->string_buffer[i++] = 0;
        }
        HSD_SisLib_Free(old_buffer);
    }
}
...
void HSD_SisLib_803A7684(HSD_Text* text, const u8* cursor, u8 flags)
{
    switch (flags & 0x7F) {
    case 1:
        HSD_SisLib_EnsureTextBuffer(text, 5);
        ...
        return;
    case 2:
        HSD_SisLib_EnsureTextBuffer(text, 4);
    ...
    case 3:
        ...
        return;
    case 4:
        HSD_SisLib_EnsureTextBuffer(text, 2);
